...
| 🔨 | **Fill a field on the canvas** — click the widget on the page and type, which is what every reader does and what the panel-only surface was not. Uses `EditSession::widget_rects(page_index)`, **shipped by pdfce on request the same day** (`e8e9881`): `/Annots` order, corners normalised, session-overlay aware, so nothing re-parses the whole AcroForm per hit-test. Two constraints fell out of building it — **a field with no drawn appearance cannot be clicked**, because you cannot click what is not painted; and the four mandatory `FillOutcome` disclosures have no canvas home under Rule 4, so they keep an off-canvas one |
