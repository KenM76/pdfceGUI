...
because `pdfce-core` says *"the shell resolved for it"* and *"pdfce never goes
looking"*. `pdfce_render::FontEnvironment::resolve_for_embedding` already had
...
`pdfce_core::FontMatch` and `pdfce_render::EmbedMatch`, same three variants,
