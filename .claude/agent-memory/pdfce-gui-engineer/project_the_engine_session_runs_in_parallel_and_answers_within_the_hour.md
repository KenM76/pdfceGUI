...
description: A pdfce session works D:\Dev\pdfce concurrently, reads the request channel and answers within minutes — so file precisely, and expect the read-only tree to show uncommitted changes that are not yours
metadata:
  type: project
---

**`D:\Dev\pdfce\` has a live session working it at the same time as this one.**
...
1. **`git status` in the read-only tree will show modifications that are not
   ...
   `D:\Dev\pdfce`, documenting the very gaps just reported. **Do not read that as
...
- Run `cargo update -p pdfce-core -p pdfce-render -p pdfce-print` at the *start*
