...
description: A sentence about what pdfce-core cannot do goes stale within hours; spell such claims as test assertions, never as prose
metadata:
  type: feedback
---
...
opposite directions. Written first from `D:\Dev\pdfce`'s *working tree* — which
...
`D:\Dev\pdfce`'s source, which is dirty by design because that session runs in
