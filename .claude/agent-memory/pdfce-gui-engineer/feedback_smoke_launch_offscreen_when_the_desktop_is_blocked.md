...
description: PDFCE_DIAG_VIEWPORT gives a real laid-out off-screen window, so a launch-and-read-the-trace check costs the operator nothing even when ui-verify is blocked
metadata:
  type: feedback
---
...
```bash
PDFCE_DIAG=1 PDFCE_DIAG_VIEWPORT=-4000,-4000,1400,900 \
  timeout 12 ./target/release/pdfce-gui.exe <fixture.pdf> > smoke.txt 2>&1
```
