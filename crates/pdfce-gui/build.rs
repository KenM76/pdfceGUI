...
/// time of the build … and the date and time of the builds of the used pdfce
/// and iccce"*.
///
/// # ★ Why the engine's stamp is a COMMIT time and not a build time
///
/// `pdfce-core`, `pdfce-render` and `pdfce-print` are compiled **into** this
...
/// else here, so a "pdfce build time" would be this binary's build time
...
///
/// `PDFCE_BUILD_STAMP`, when set, wins. `tools/package-portable.py` sets it,
...
/// a dependency that is not already in pdfce's lockfile (`PROJECT_PLAN.md` §2),
...
fn provenance() {
    ...
    println!("cargo:rerun-if-env-changed=PDFCE_BUILD_STAMP");

    let stamp = std::env::var("PDFCE_BUILD_STAMP").unwrap_or_else(|_| utc_now());
    println!("cargo:rustc-env=PDFCE_BUILD_TIME={stamp}");
    ...
    let (version, rev, repo) = locked_git_package(&lock, "pdfce-core");
    println!("cargo:rustc-env=PDFCE_ENGINE_VERSION={version}");
    println!(
        "cargo:rustc-env=PDFCE_ENGINE_REV={}",
    ...
    println!("cargo:rustc-env=PDFCE_ENGINE_TIME={engine_time}");
    ...
    println!("cargo:rustc-env=PDFCE_ICCCE_VERSION={icc_version}");
    println!(
        "cargo:rustc-env=PDFCE_ICCCE_REV={}",
    ...
    println!("cargo:rustc-env=PDFCE_ICCCE_TIME={icc_time}");
}
...
///
/// ★ Reading `D:\Dev\pdfce` is deliberate and permitted: the governing rule of
...
/// name = "pdfce-core"
/// version = "0.7.0"
/// source = "git+file:///D:/Dev/pdfce?branch=main#6af5655c…"
/// ```
///
    ...
    for line in lock.lines() {
        ...
        if let Some(v) = line.strip_prefix("source = ") {
            ...
            if let Some((url, sha)) = src.split_once('#') {
                rev = sha.to_owned();
                // `git+file:///D:/Dev/pdfce?branch=main` -> `D:/Dev/pdfce`
...
/// because the workspace may add none that pdfce's lockfile does not already
