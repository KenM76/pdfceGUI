...
| file | how it ships |
|---|---|
| `pdfce-gui.ico` | compiled into `pdfce-gui.exe`'s `.rsrc` section by `pdfce-gui.rc`, via `build.rs` |
| `pdfce-gui.rc` | the resource script that does it — source, not shipped, but kept here with what it names |
...
(`crates/pdfce-gui/src/icons/assets/PROVENANCE.md`,
`crates/pdfce-gui/src/app/assets/PROVENANCE.md`) and of `D:\Dev\pdfce`'s own,
...
- **Source:** authored for pdfceGUI on 2026-08-18 by a Claude Code build
...
1. **A change is a diff.** A checked-in `.ico` is art nobody can review or
   ...
   editable.
2. **The two artefacts cannot drift.** `pdfce-gui.ico` and
...
- **the simple route** — replace `pdfce-gui.ico` with any multi-size `.ico`,
  delete `tools/make-icon.py`, and amend this file to name the new author and
  terms. Nothing in `build.rs` or `pdfce-gui.rc` needs to change;
...
`pdfce-gui.rc` embeds a `VERSIONINFO` block as well as the icon, so the
