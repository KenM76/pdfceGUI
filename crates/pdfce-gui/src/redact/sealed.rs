...
//! crate**:
//!
//! > [`pdfce_core::redact::apply_redactions`] is *called* in exactly one place
...
//! pdfce_core::redact::apply_redactions(&doc, &opts)?; std::fs::write(p,
...
//!
//! It is not a hypothetical. `pdfce-cli`'s `redact-apply` does precisely that
...
//! other Rust in this workspace that could reach `pdfce-core` is a harness that
...
/// spelling decision (`pdfce_core::redact::apply_redactions` today, a `use`
    ...
    fn the_apply_pipeline_never_reaches_for_the_incremental_writer() {
        ...
        assert!(
            ...
            "★ `{FORBIDDEN_IN_REDACT}` is called inside `redact/`: {:?}\n\
             ...
             one `startxref` hop away in a document pdfce has told the operator \
...
use pdfce_core::redact::apply_redactions;
...
"####;
    ...
    fn a_planted_second_call_is_reported() {
        let planted = format!(
            "{FIXTURE}\nfn sneaky() {{ let f = || {{ let _ = \
             pdfce_core::redact::apply_redactions(&d, &o); }}; f(); }}\n"
        );
        assert_eq!(
    ...
    fn the_walker_descends_and_reports_a_planted_file() {
        let root = std::env::temp_dir().join("pdfce-gui-sealed-selftest");
