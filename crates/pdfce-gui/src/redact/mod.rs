...
//!
//! Both halves were re-checked against `D:\Dev\pdfce` on 2026-08-15 rather than
//! quoted: [`pdfce_core::redact::apply_redactions`] still returns
...
//! `pdfce-core`. The request channel is empty, so nothing is owed and Pass 72.0
...
//!
//! [`pdfce_core::redact::apply_redactions`] takes a `&Document` — a parsed file
...
//! enforced by the **caller** remembering to run it. `pdfce-cli`'s
...
//! doc comment quoting `[`pdfce_core::redact::apply_redactions`]` is not a call
...
//!
//! None of this stops a future author writing `pdfce_core::redact` calls in a
...
//! in [`pdfce_core::redact`] and are called, never re-derived — the GUI/core
...
/// `pdfce-gui.exe`. See this crate's `Cargo.toml` for why a real parser and not
...
pub enum RedactApplyRefusal {
    ...
    /// produced something the parser rejects, which is a pdfce bug rather than
    /// a property of the operator's file.
    MaterialisedDocumentUnreadable {
        /// The parse diagnostic.
        reason: String,
    },
    /// `pdfce-core` refused the apply itself: a region over a raster image it
...
pub struct PreparedRedaction {
    ...
    /// bytes()` here would restore exactly the surface `pdfce-cli`'s
...
impl PreparedRedaction {
    ...
    /// expected state) and the other is *"pdfce and pdfce disagree about
...
///
/// # ★ This is the one call site of [`pdfce_core::redact::apply_redactions`] in
/// this crate
///
...
mod tests {
    ...
    ///
    /// The same fixture shape `pdfce-core`'s own redaction tests use —
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join("pdfce-gui-redact-tests");
    ...
    /// text must not be recoverable from the saved bytes by any means pdfce
    /// itself offers. Three independent measures, because a single one could be
    /// satisfied by a build that merely hid the text:
    ///
    /// 1. **`extract-text`** — the very tool `pdfce-cli extract-text` and this
    ...
    fn applied_redaction_leaves_no_recoverable_trace_in_the_saved_bytes() {
        ...
        // (1) pdfce's own text extraction — the tool an operator would actually
