...
//!
//! [`pdfce_core::redact::apply_redactions`] returns a **[`RedactionReport`]** —
...
//! `RedactionVerdict` and no `verify_redaction` anywhere in `pdfce-core`
...
//!
//! [`RedactionReport`]: pdfce_core::redact::RedactionReport
//!
//! ## The three verdicts, and why the middle one is not a refusal
//!
//! [`pdfce_core::redact::RedactionReport::redacted_text`] carries the distinct
...
/// reason worth stating plainly: these are bytes pdfce itself just wrote, so an
...
///
/// The same naive scan `pdfce-core`'s own absence tests use, kept local rather
...
mod tests {
    ...
    fn a_raw_byte_run_outside_every_stream_is_disclosed_rather_than_refused() {
        ...
        assert_eq!(
            ...
            "…and it must be DISCLOSED rather than passed over, because pdfce \
