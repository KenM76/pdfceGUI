...
//! rewriting:
//!
//! > pdfce diverges from Acrobat here on purpose — Acrobat clips silently
//! > when content falls outside the printable area, and pdfce says so. That
//! > divergence is worth nothing if the GUI reduces it to a count an operator
//! > can look past.
//! >
//! > The whole reason `pdfce-print` reads real device geometry instead of
...
const PREVIEW_TEXTURE_ID: &str = "pdfce-print-preview"; // ui-text-exempt: internal texture id, never displayed
...
/// rotates page content either: the driver turns the sheet, pdfce does not
...
fn texture_for(
    ...
    let rendered =
        pdfce_render::render_page_with_view(&view, page_obj, raster_scale(size), &options).ok();
...
/// one of them failing.
fn upload(ctx: &egui::Context, pixmap: &pdfce_render::tiny_skia::Pixmap) -> TextureHandle {
