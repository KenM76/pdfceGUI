...
impl PrintRange {
    ...
    /// applied here: they are `pdfce-print`'s, they have a defined order of
...
pub(super) fn copies_finishing(ui: &mut Ui, dialog: &mut PrintDialog) {
    ...
    // ★ R83: no duplex control for a device that cannot duplex. pdfce does
...
pub(super) fn comments_resolution(
    ...
    // pdfce's own fifth value, load-bearing for the round-trip raster oracle
    // rather than for an operator, and a print with neither form fields nor
    // links is not a thing this dialog has been asked for.
    for (scope, label) in [
        (pdfce_render::AnnotationScope::Document, t::scope_document()),
        (
    ...
    // Conditional, because it is a per-job substitution: pdfce picked a
