...
//! ## The gap
//!
//! Found **2026-08-28**, auditing this build against `pdfce-core`'s capability
...
//! signature pdfce believed to be invalidated, and pdfce kept that belief to
//! itself.
//!
...
//!   signature"*). pdfce can state the outcome as fact.
//! * **`ConservativeReport`** — only approval signatures are present, and ISO
//!   32000-1 defines stage 1 and only stage 1 for them. pdfce reports
...
use pdfce_core::signature::{ImpactBasis, SaveMode, SignatureImpact};
...
impl SignatureDialog {
    ...
    /// 2. **the footing** — why pdfce says so, and whose claim it is;
    ...
    ///    operator uses;
    /// 5. **the footnote** — that pdfce verified nothing — below the buttons,
...
mod tests {
    ...
    /// pdfce is unsure what it just asserted.
    ///
    ...
    /// wording that asserts **less**: pdfce stating Table 254 as fact about a
    ...
    fn the_signed_fixture_moves_from_a_note_to_a_window_when_a_page_goes() {
        ...
        assert_eq!(
            ...
            "a page removed from a signed document must raise the question, on the footing that \
             says the verdict is pdfce's rather than the standard's"
        );
        assert_eq!(count, 1);
