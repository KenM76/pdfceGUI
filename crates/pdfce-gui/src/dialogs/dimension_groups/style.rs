...
//! ## What this draws
//!
//! The **middle tier** of `pdfce-core`'s three-tier style cascade
...
//! *this group has not spoken, so pdfce's own default applies*, ticked means
...
//!
//! `pdfce-core` pins that with a test named for the trap
...
/// end. That is the CLI's own convention (`pdfce-cli group-style`: setting one
...
fn will_move(
    ...
    group: &Group,
    pick: impl Fn(&pdfce_core::dimension::StyleProvenance) -> StyleSource,
) -> usize {
    model
        .members(group.id)
...
/// would be a channel pdfce silently ignores — the argument
...
mod tests {
    use super::*;
    use pdfce_core::dimension::{DEFAULT_GROUP_ID, DimensionKind, StyleOverrides, Unit};
