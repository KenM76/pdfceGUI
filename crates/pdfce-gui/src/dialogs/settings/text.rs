...
//!
//! `pdfce-core` is explicit: *a redaction built under one value is not
...
///
/// `MIN_WORD_GAP_RATIO` and `MAX_WORD_GAP_RATIO` are `pub` in `pdfce-core`
...
/// What stands in for text pdfce cannot decode.
///
...
///
/// The larger half is documented in `pdfce-core` and was shown nowhere: the
...
///
/// `pdfce-core` calls it *"a fact to disclose, not a direction to pick"*, and
