...
//!
//! `Settings::theme` is an **opaque token** in `pdfce-core`, deliberately.
//! The set of themes is a shell concern and `pdfce-core` must never gain a GUI
...
pub fn theme(ui: &mut Ui, draft: &mut Draft) {
    ...
    // settings file from a NEWER pdfce, whose token is being **preserved**
...
mod tests {
    ...
    ///
    /// `pdfce-core` writes `"quiet"` as a **literal** in `Settings::default`,
    ...
    /// newer pdfce must still carry it after the window has drawn — this test
