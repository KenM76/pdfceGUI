...
use pdfce_core::settings::MissingAppearanceState;
...
/// ticked box, an "off" stamp — **with no indication that pdfce chose it**. The
...
///
/// Whatever is chosen, the case stays **counted**, and pdfce never writes an
/// `/AS` to repair the file — the document is malformed and pdfce's job is to
...
///
/// It uses the dictionary's own iteration order, which pdfce preserves from the
