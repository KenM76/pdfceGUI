...
pub struct EmbedDialog {
    ...
    /// travel together.
    request: pdfce_core::font_embed_missing::EmbedRequest,
...
impl EmbedDialog {
    ...
    pub fn open(doc: &OpenDoc, folders: &[std::path::PathBuf]) -> Option<Self> {
        ...
        // "missing" means.
        let mut request = pdfce_core::font_embed_missing::EmbedRequest::all_missing();
        ...
        let shown: Vec<usize> = plan
            ...
            .filter(|(_, b)| {
                !matches!(
                    b.blocker,
                    pdfce_core::font_embed_missing::EmbedBlocker::AlreadyEmbedded
                )
            })
            .map(|(index, _)| index)
            .collect();
