...
pub struct UnembedDialog {
    ...
    request: pdfce_core::font_unembed::UnembedRequest,
...
impl UnembedDialog {
    ...
    pub fn open(doc: &OpenDoc) -> Option<Self> {
        ...
        let request = pdfce_core::font_unembed::UnembedRequest::all_removable();
        ...
        let shown: Vec<usize> = plan
            ...
            .filter(|(_, blocked)| {
                !matches!(
                    blocked.blocker,
                    pdfce_core::font_unembed::UnembedBlocker::NotRemovable(
                        pdfce_core::fontinfo::Removability::NotEmbedded
                    )
                )
            })
            .map(|(index, _)| index)
