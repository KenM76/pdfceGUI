...
//!
//! `pdfce-core`'s `export::dxf` has shipped the whole time, and the old shell
...
//!
//! pdfce can do better than guess because it already has the operator's own
...
pub struct ExportDxfDialog {
    ...
    page_index: usize,
    /// What pdfce inferred, kept so the disclosure can be redrawn without
...
impl ExportDxfDialog {
    ...
    pub fn open(doc: &OpenDoc) -> Self {
        ...
        // pdfce answering a question it has just said it cannot answer, and the
    ...
    /// What pdfce inferred, and what the operator should make of it.
    ///
    ...
    fn scale_disclosure(&mut self, ui: &mut Ui) {
        match &self.suggestion {
            ...
            // the same honest sentence as "pdfce does not know" rather than
            // nothing at all.
