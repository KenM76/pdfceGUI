...
//! touching this file. In short: nothing in `pdfce-core` wrote a `/MediaBox`,
...
//!
//! `EditSession::set_media_box` and `pdfce_core::paper` shipped, so the whole
...
//! sheet in pdfce, so the *second* use of this dialog is rare enough that a
...
//! number in it. The one thing pdfce decides on their behalf is the **refusal**
...
/// therefore 1.7-era portability advice pdfce is choosing to honour.
const MAX_CUSTOM_MM: i64 = 5080;

/// Which sheet the operator has picked.
///
/// A separate type from `pdfce_core::paper::PaperSize` rather than
...
enum Choice {
    /// One of `pdfce_core::paper::PaperSize::ALL`.
    Standard(pdfce_core::paper::PaperSize),
...
impl NewDocumentDialog {
    ...
    pub fn open() -> Self {
        Self {
            choice: Choice::Standard(pdfce_core::paper::PaperSize::A4),
            landscape: false,
    ...
    fn sheet_pt(&self) -> (f64, f64) {
        let orientation = if self.landscape {
            pdfce_core::paper::Orientation::Landscape
        } else {
            pdfce_core::paper::Orientation::Portrait
        };
        match self.choice {
    ...
    fn body(&mut self, ui: &mut Ui) {
        ...
        let combo = egui::ComboBox::from_id_salt("new-document-size")
            ...
            .show_ui(ui, |ui| {
                ...
                for (index, size) in pdfce_core::paper::PaperSize::ALL.iter().enumerate() {
        ...
        // `pdfce_core::paper`'s own argument for keeping `Orientation` a
...
mod tests {
    ...
    fn landscape_transposes_a_standard_size_and_a_custom_one() {
        ...
        dialog.choice = Choice::Standard(pdfce_core::paper::PaperSize::A1);
    ...
    fn a_standard_sheet_is_the_engines_own_number() {
        ...
        let expected = pdfce_core::paper::PaperSize::A1.size_pt();
        assert!(
