...
//! against `pdfce-core`'s API for `OPERATOR_REQUESTS.md` O108.
//!
...
//! [`crate::app::lifecycle::PdfceApp::open_path`] branches on it by structured
//! error data rather than by message, and produces
//! [`crate::app::state::Status::NeedsPassword`] — a tab whose tooltip reads, in
//! this build, *"This document is encrypted and pdfce has not been given the
//! password."*
//!
...
//! - **No attempt limit and no delay.** pdfce is reading a local file the
...
//! stderr under `PDFCE_DIAG`, and `tools/ui-verify` **captures that stderr to a
...
///
/// ★ Two variants, not one, because `pdfce-core` reports two errors and its own
...
pub enum Rejection {
    ...
    /// normalisation pdfce does not implement. **The password may be correct.**
    NeedsNormalisation,
}
...
mod tests {
    ...
    ///
    /// The whole of [`PasswordDialog::submit`]'s argument, asserted: pdfce has
    ...
    fn the_normalisation_refusal_is_not_the_wrong_password_refusal() {
        ...
        assert_ne!(
            ...
            "the two failures must not read the same: one means try again, the other \
             means pdfce cannot open this file however correct the password is"
        );
    }
}
