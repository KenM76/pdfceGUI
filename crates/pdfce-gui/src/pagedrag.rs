...
//!
//! ## ★ Why `egui::Memory` and not a field on `PdfceApp`
//!
...
pub struct PageDrag {
    ...
    /// every consumer needs to compare against `PdfceApp::active_slot` and
    /// what `PdfceApp::slot` takes. Slot positions are stable while a drag is
...
fn active_key() -> egui::Id {
    egui::Id::new("pdfce-active-document") // ui-text-exempt: an id, never displayed
}
...
/// after the last. `pdfce_core::pageops::InsertPosition` counts pages, so the
...
#[must_use]
pub fn insert_position(gap: usize, page_count: usize) -> pdfce_core::pageops::InsertPosition {
...
fn key() -> egui::Id {
    egui::Id::new("pdfce-page-drag") // ui-text-exempt: an id, never displayed
}
...
fn landing_key() -> egui::Id {
    egui::Id::new("pdfce-page-drag-landing") // ui-text-exempt: an id, never displayed
}
...
fn landing_shown_key() -> egui::Id {
    egui::Id::new("pdfce-page-drag-landing-shown") // ui-text-exempt: an id, never displayed
}
