...
//! binary
//!
//! `PDFCE_DIAG=1 pdfce-gui <file>`, release build, reading the
...
//!   their documents is the expensive kind. pdfce can, after one page, and
...
//!   what pdfce owes them is the number, not the decision.
...
/// thread that trips at this deadline. `pdfce-render` polls the token
...
pub enum Unavailable {
    ...
    Abandoned,
    /// `pdfce-render` refused the page. The string is the renderer's own
...
pub struct ThumbnailCache {
    ...
    /// honest. `None` is *"pdfce is deciding"* — on until a page proves
...
impl ThumbnailCache {
    ...
    /// direction. Their choice is then the reason, and printing pdfce's
    /// explanation beside it would credit the wrong party — the note this
    /// feeds ([`crate::text::pages::previews_paused_note`]) reads as
    /// *"pdfce stopped"*, which stops being true the moment they touch the
    /// control.
    #[must_use]
    ...
    pub fn render(
        ...
        let outcome = {
            ...
            pdfce_render::render_page_with_view(&view, page, scale, &options)
        };
...
mod tests {
    ...
    /// page that rendered in four milliseconds — pdfce inventing evidence for
    ...
    ///
    /// An infinite scale reaches `pdfce-render`'s pixmap guard and comes back
