...
//! that does not exist renders nothing."* `pdfce-core` `Pass 161.0` shipped
...
//! | it is **dimmed**, never hidden, where the drop would change nothing | *"drawing no caret cannot be told apart from the panel having stopped tracking the pointer — and the no-op boundary is where every drag begins"* | unchanged, and it carries a second dimmed state for a drop pdfce will refuse |
...
use pdfce_core::edit::OutlinePlacement;
...
/// nothing"* is answered by letting go somewhere else at leisure; *"pdfce will
...
///
/// A refusal is the operator asking for something pdfce will not do, and **a
...
/// **Would this move do anything, and would pdfce allow it?**
///
...
pub fn landing_for(items: &[OutlineItem], dragged: ObjId, to: OutlinePlacement) -> Landing {
    ...
    // operator needs is the one that says pdfce will not do it.
...
mod tests {
    ...
    /// would keep repeating the drop that pdfce will never accept.
    #[test]
