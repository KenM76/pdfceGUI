...
//!
//! That value comes from `pdfce-core`'s own `Layer::visible_by_default` —
...
//! without it the operator's only available reading is *"pdfce got it
//! wrong"*:
//!
...
//! | that radio group contains a **locked** member | pdfce will not switch that sibling off, so this group can end up with two members showing. See "`DA-A8`" below. |
//!
...
//!    [`pdfce_render::LayerVisibility`] exists and `pdfce-render` honours
//!    content-stream `/OC`.
...
//!    `PdfceApp::apply`. Before that a panel body — handed `&OpenDoc`, a
...
//! — and `pdfce_core::layers` hands the panel everything needed to honour
//! them *before the first click*: [`pdfce_core::layers::Layer::radio_group`]
//! is an index into [`pdfce_core::layers::Layers::radio_groups`], which
...
//!
//! "At most one" permits none. Picking a replacement would be pdfce choosing
...
//!
//! `pdfce_core::layers` names this as a genuine gap in the standard and hands
...
//!
//! **pdfce lets the lock win**: [`toggle_actions`] skips a locked sibling, so
...
//! against `pdfce_core::annot::optional_content_default_off`, **not** by
...
pub fn body(ui: &mut egui::Ui, doc: &OpenDoc, _state: &mut PanelsState, actions: &mut Vec<Action>) {
    ...
    let read = pdfce_core::layers::read_layers(&view);
    ...
    let document_hidden = pdfce_core::annot::optional_content_default_off(&view);
...
/// group is being examined, over at most [`pdfce_core::layers::MAX_LAYERS`]
...
///   permits none, and choosing a replacement would be pdfce deciding which
...
///   would be pdfce deciding something ISO declined to.
///
...
///
/// A pure function over one [`pdfce_core::layers::Layer`] and the effective
...
/// concluding "pdfce got it wrong", and a row that silently loses its
...
fn row_caveats(
    ...
    // tell whether that is intent filtering or a pdfce bug. pdfce inferred
...
///
/// [`pdfce_core::layers::Layer::radio_group`] is an *index* into
/// [`pdfce_core::layers::Layers::radio_groups`] rather than a member list, so
...
mod tests {
    ...
    fn read_fixture(rel: &str) -> (Layers, BTreeSet<ObjId>) {
        ...
        let read = pdfce_core::layers::read_layers(&doc);
        let hidden = pdfce_core::annot::optional_content_default_off(&doc);
        (read, hidden)
    }
    ...
    fn the_render_key_no_longer_blocks_a_layer_toggle() {
        ...
        let mut open =
            crate::app::state::OpenDoc::new(path, pdfce_core::edit::EditSession::new(doc), pages);

        // Exactly what the panel body reads.
        let read = pdfce_core::layers::read_layers(&open.session.view());
        assert!(
    ...
    fn a_composed_click_changes_both_the_hidden_set_and_the_render_key() {
        ...
        // … and what `PdfceApp::apply` does with each of them.
        for a in &raised {
            if let Action::SetLayerVisible { group, visible } = a {
    ...
    fn the_reset_control_appears_exactly_when_the_view_differs_from_the_document() {
        ...
        let document_hidden = pdfce_core::annot::optional_content_default_off(&open.session.view());
    ...
    /// has no way to know pdfce did that rather than the document.
    ///
    ...
    ///
    /// pdfce's answer to a conflict the standard leaves open, asserted rather
    ...
    ///
    /// "At most one" permits none. Choosing a replacement would be pdfce
    ...
    /// constraints are not jointly satisfiable. `pdfce_core` answers with
            ...
            for action in toggle_actions(&read, l.id, true) {
                ...
                assert!(
                    ...
                    "clicking {:?} reached {group:?}, which core did not report \
                     as a member of its radio group — pdfce has invented a \
    ...
    /// reading of a surprising row is "pdfce got it wrong". Two that read
