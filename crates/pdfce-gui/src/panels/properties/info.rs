...
//! > built here; the first needs a `/Info` accessor that `pdfce-core` does not
...
//! > actually changed it.
//!
//! A `/Title` written in an encoding pdfce cannot fully resolve comes back
...
//! own bytes with pdfce's guess at them**, silently, in a field nobody looks
//! at twice.
//!
...
//!    fact about the **document**, not a pdfce failure, and is worded that way.
//!
...
/// string.
///
/// What is deliberately absent is anything pdfce would have to **infer** —
...
fn row(
    ...
    ui.horizontal(|ui| {
        ...
        // obligation: a field pdfce could not decode exactly is never written
...
mod tests {
    ...
    /// `InfoField` added to `pdfce-core` changes `InfoField::all()`'s return
