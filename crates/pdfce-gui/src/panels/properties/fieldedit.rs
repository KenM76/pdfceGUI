...
//! is delete-and-recreate. pdfce models the same limit by making the request
...
use egui::Ui;
use pdfce_core::edit::FieldEdit;
use pdfce_core::forms::{Field, FieldFlags, FieldType};
...
fn tooltip_row(ui: &mut Ui, fqn: &str, state: &mut PanelsState, actions: &mut Vec<Action>) {
    ...
    if lost && typed != stored {
        let choice = if typed.is_empty() {
            ...
            pdfce_core::edit::TooltipChoice::Text(typed)
        };
        actions.push(
            FieldAction::EditProperties {
                field: fqn.to_owned(),
