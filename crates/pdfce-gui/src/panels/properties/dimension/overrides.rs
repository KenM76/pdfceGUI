...
///
/// Powers of two only. `pdfce-core` accepts any `u32`, and a drawing
...
pub fn show(ui: &mut Ui, group: &Group, overrides: &mut StyleOverrides) -> bool {
    ...
    let provenance = pdfce_core::dimension::style_provenance(group, overrides);
...
mod tests {
    ...
    /// property to `pdfce-core` without a row in this file fails this test with
    ...
    fn no_property_of_the_cascade_is_left_without_a_row() {
        ...
        let provenance =
            pdfce_core::dimension::style_provenance(&group, &StyleOverrides::default());
    ...
    /// `Factory`, the row would show *"using pdfce's default"* for a property
    ...
    fn the_four_concrete_properties_never_claim_a_factory_source() {
        let group = Group::new(GroupId(0), "Plan", Unit::Millimeter);
        let p = pdfce_core::dimension::style_provenance(&group, &StyleOverrides::default());
        for (name, source) in [
            ("unit", p.unit),
            ("fraction", p.fraction),
