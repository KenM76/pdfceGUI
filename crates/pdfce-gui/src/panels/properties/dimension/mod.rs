...
//!
//! and `pdfce`'s `FEATURES.md`, which carries the same row as
...
use egui::Ui;
use pdfce_core::dimension::{DimensionKind, DimensionRecord};
...
pub fn section(ui: &mut Ui, doc: &OpenDoc, actions: &mut Vec<Action>) -> bool {
    ...
    let Some(group) = model.group(record.group) else {
        ...
        // impossible through any pdfce path — the sidecar carries both — and
    ...
    if let Some(display) = model.display(record.id) {
        ...
        if display.raw_page_units {
            ...
            ui.weak(pdfce_core::dimension::NO_SCALE_DISCLOSURE);
        }
    }
