...
//! `pdfce-core` had no verb — every colour verb it owned worked on an
...
//! | `Other { space, … }` | a space pdfce does not decode — a spot ink, a pattern | **the ink's NAME, and no swatch** |
//!
...
//!
//! Evaluating a tint transform lives in `pdfce-render`, which `pdfce-core`
...
use egui::Ui;
use pdfce_core::vector::PathPaint;
...
pub fn section(ui: &mut Ui, doc: &OpenDoc, actions: &mut Vec<Action>) -> bool {
    ...
    let Some(pdfce_core::vector::VectorObject::Path(path)) = model.objects.get(object) else {
...
fn to_bytes(rgb: pdfce_core::vector::Rgb) -> [u8; 3] {
...
mod tests {
    ...
    fn nobody_chose_and_somebody_chose_black_both_show_black() {
        ...
        let chosen = PathPaint::Device {
            space: pdfce_core::vector::DevicePaintSpace::Gray,
            comps: vec![0.0],
            rgb: Rgb::BLACK,
        };
