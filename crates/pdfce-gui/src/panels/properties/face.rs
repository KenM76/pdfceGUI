...
//! > pdfce authors the font resource on demand, with widths, embedding
...
//! | [`FaceOrigin::PdfceWouldAdd`] | rewrites the `Tf` operand **and writes a new `/Font` object** | a dictionary with widths and no glyph outlines, plus a face drawn from the reader's own copy |
//!
...
//!
//! * the page-`/Contents` path (`pdfce-core/src/edit.rs` ~7887) takes
...
//! conditions), and a second copy of the rule in `pdfce-gui` would drift from the
...
//! authors a face when the lookup misses. Offering `Helvetica` as *"pdfce can
...
/// way and because a third origin is foreseeable: `Pass 142.0` would let pdfce
...
pub(crate) enum FaceOrigin {
    ...
    /// page does **not** carry and which pdfce would author on demand.
    ///
...
pub(crate) struct FaceChoice {
    ...
    ///
    /// For a [`FaceOrigin::PdfceWouldAdd`] row it is the exact §9.6.2.2
...
/// resource and refuse it — while a row labelled *"pdfce can add Helvetica"* had
...
/// written to avoid.
pub(crate) fn choices(preflight: Option<&pdfce_core::text_edit::FontPreflight>) -> Vec<FaceChoice> {
    ...
    rows.extend(
        Std14::ALL
            ...
            .map(|name| FaceChoice {
                ...
                label: name.to_owned(),
                ambiguous: false,
                origin: FaceOrigin::PdfceWouldAdd,
            }),
    );
    rows
}
...
pub(crate) fn popup_body(
    ...
    for face in faces {
        ...
        match face.origin {
            ...
            FaceOrigin::PdfceWouldAdd if !drew_addable_heading => {
        ...
        if face.origin == FaceOrigin::PdfceWouldAdd && !published_first_addable {
...
mod tests {
    ...
    /// page built from `ArialMT` alone offered exactly one face and pdfce's
    ...
    /// constructed outside `pdfce-core`. What that costs is coverage of
    ...
    /// `accepted()`, so a filter built on that list would offer *"pdfce can add
    ...
    fn the_two_origins_are_not_equal() {
        assert_ne!(FaceOrigin::OnThisPage, FaceOrigin::PdfceWouldAdd);
    }
}
