...
//! opacity for a **placed** markup"*, and the row `pdfce`'s own capability
...
use egui::Ui;
use pdfce_core::annot_author::Color;
use pdfce_core::edit::{MarkupStyle, StyleEdit};
...
pub fn section(ui: &mut Ui, doc: &OpenDoc, actions: &mut Vec<Action>) -> bool {
    ...
    // exactly the case where making the controls absent would read as pdfce
...
///
/// `pdfce_core::annot::Annotation` is the **reader's** view — id, subtype,
...
/// to guess"* — geometry that is missing, or is not something pdfce models. A
...
impl Current {
    ...
    fn read(doc: &OpenDoc, id: pdfce_core::object::ObjId) -> Self {
        use pdfce_core::annot_author::{MarkupSpec, spec_from_dict};
...
fn width_row(
    ...
    // capability renders nothing. A greyed spinner here would be pdfce
