...
//!
//! `pdfce-core` is built for it: `Removability`'s own doc comment says the
//! unembedding verb *"consumes this exact value, so a font pdfce shows as
//! blocked and a font pdfce declines to unembed are the same set by
//! construction."*
//!
...
//! as an unconfirmed gap, so pdfce states its own scope rather than assuming
//! parity with a behaviour nobody has measured.
//!
//! The page-scan failure sits above even that, because it changes what an
//! empty list beneath it *means*: without it, "0 fonts" and "pdfce could not
...
//!
//! Both push a mutation through `pdfce_core::edit::EditSession`, and at S3
...
//! the **file**, and error styling would make it read as a pdfce failure.
...
pub fn body(
    ...
    use pdfce_core::fontinfo::{Program, Removability};
    ...
    // about a plan pdfce could carry out, which is what makes them safe to
...
fn row_body(ui: &mut egui::Ui, f: &pdfce_core::fontinfo::FontRecord) {
...
/// absent case, so pdfce defines none either: unknown says the word
...
fn fs_type_lines(ui: &mut egui::Ui, fs: &pdfce_core::fontinfo::FsType) {
