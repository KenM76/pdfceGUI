...
use pdfce_core::annot_author::{Color, RedactAppearance};
...
pub enum Fill {
    /// A solid black box. The convention, and what pdfce has always drawn.
    Black,
    ...
    /// `pdfce_core::annot_author::Color::Rgb`, so no conversion happens
...
fn controls(ui: &mut egui::Ui, appearance: &mut Appearance) {
    ...
    // reserves for something pdfce owes the operator rather than something it
