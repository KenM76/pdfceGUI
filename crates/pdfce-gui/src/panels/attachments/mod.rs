...
//!
//! `pdfce-core` has carried `attach_file`, `detach_file`, `list_attachments`,
...
//!   and `pdfce-core` has no verb that changes one afterwards. R9: an absent
...
//!   somewhere, and `pdfce_core::attachments`' module docs are explicit that
//!   pdfce *"does not execute, open, or interpret them, and neither should a
...
//!
//! Each is an obligation `pdfce-core` writes into its own doc comments, and
...
//!    `DefEmbeddedFile` crypt filter, §7.6.5), and pdfce does not decrypt on
//!    this path — so an extraction can succeed and produce garbage, silently.
//! 3. **The name pdfce writes to disk may not be the name shown.** The listing
...
//! picker lives in `PdfceApp::apply`. See
...
///    the document holds two files; the sentence saying pdfce stopped reading
...
pub fn body(ui: &mut Ui, doc: &OpenDoc, state: &mut PanelsState, actions: &mut Vec<Action>) {
    let (listed, notes) = {
        ...
        pdfce_core::attachments::list_attachments_with_notes(&view)
    };
...
/// One attachment: what it is, what pdfce could not vouch for, and what can be
/// done with it.
fn row(
    ui: &mut Ui,
    ...
    // disclosure for the other half of the bargain, which is that pdfce never
    ...
    // What pdfce could not vouch for about this row, in the order an operator
...
/// pdfce cannot name gets no button. That is R9 rather than caution: a control
...
/// this crate's rule 4 posture is otherwise to disclose everything: pdfce is
...
mod tests {
    ...
    fn both_kinds() -> Vec<Attachment> {
        ...
        pdfce_core::attachments::list_attachments(&doc)
    }
    ...
    fn only_a_nameable_attachment_gets_a_verb() {
        ...
        let unnameable = AttachmentKind::PageAnnotation {
            page_index: 0,
            page_id: pdfce_core::object::ObjId::new(1, 0),
            annot_id: None,
            icon: None,
        };
    ...
    /// suspicious file would be looking at pdfce's cleaned-up version"*; the
    ...
    fn a_hostile_name_is_shown_and_not_quietly_repaired() {
        ...
        let doc = pdfce_core::document::Document::load(&path).expect("the fixture loads");
        let listed = pdfce_core::attachments::list_attachments(&doc);
