...
//! shipped in `pdfce-core` `Pass 173.0` and this shell named **none** of them.
//! So an attachment could not be moved from one open document to another, which
//! is an odd thing to be missing now that pdfce is multi-document — and nothing
...
/// is reachable.
pub(crate) fn put(ctx: &egui::Context, clip: pdfce_core::attachments::AttachmentClip) {
    crate::diag::trace(|| {
        // ui-text-exempt: diagnostic trace, never displayed
        format!(
