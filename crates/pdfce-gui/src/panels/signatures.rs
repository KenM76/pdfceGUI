...
//! pdfce does not check whether they are valid."*), because a control
...
//! bytes — which is why
//! [`pdfce_core::signature::byte_range_coverage`] takes the length as a
...
//! moment anything appended to it — including pdfce's own incremental save.
//!
...
//!
//! Signature verification is *"part 3"* in `pdfce-core`'s own consumer map,
//! and nothing about a signature is editable from anywhere in pdfce. The
...
pub fn body(
    ...
    let coverage = pdfce_core::signature::byte_range_coverage(&doc.session.view(), meta.len());
...
mod tests {
    ...
    /// **"No signatures" and "pdfce could not measure the file" are
    /// different sentences.**
    ///
    /// The first is a statement about the document. The second is a
    /// statement about pdfce's ability to look, and rendering it as the
