...
//! that pdfce knows it is there; a row that vanishes teaches nothing, while a
...
//! ```
//!
//! `pdfce_core::forms::Widget::on_states` is documented as *"the button
...
//! read says whether `/AP` `/N` `/Off` exists. (That is a `pdfce-core`
...
pub(super) fn row(
    ...
    match (field.field_type, field.button_kind) {
        ...
        // for a field whose type the document never stated would be pdfce
        // guessing what it is.
        _ => {}
    }
}
...
///
/// **Correctness, not fidelity.** pdfce cannot author `/RV`, and appearance
    ...
    let response = if multiline {
        ui.add(
            egui::TextEdit::multiline(draft)
                .id_salt(("pdfce-forms-text", index))
...
fn radio_row(ui: &mut egui::Ui, field: &Field, index: usize, out: &mut Vec<FormEdit>) {
    ...
    ui.push_id(("pdfce-forms-radio", index), |ui| {
        for state in &states {
...
fn choice_row(ui: &mut egui::Ui, field: &Field, index: usize, out: &mut Vec<FormEdit>) {
    ...
    if field.flags.has(FieldFlags::MULTI_SELECT) {
        ...
        ui.push_id(("pdfce-forms-choice-multi", index), |ui| {
    ...
    egui::ComboBox::from_id_salt(("pdfce-forms-choice", index))
...
fn choice_selections(field: &Field) -> Vec<String> {
    match &field.value {
        FieldValue::Choice(items) => items
            ...
            .collect(),
        FieldValue::Text(b) => vec![pdfce_core::edit::decode_text_string(b).text],
        _ => Vec::new(),
    }
}
