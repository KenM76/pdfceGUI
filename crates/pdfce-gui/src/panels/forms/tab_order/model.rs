...
//!
//! `pdfce_core::edit::EditSession::widget_rects`' own doc comment states both
...
//! absent in the wild, and `pdfce-core` reads it without resolving through the
...
//! eleven form fixtures in `D:\Dev\pdfce\fixtures\synthetic\forms\` write `/P`
//! on every widget. The direction is therefore inverted here as it is there:
//! each **page** is asked which widgets it lists. `pdfce_core::forms::Widget`'s
...
//! correction
//!
//! Both `pdfce-core` (`edit.rs:7508`, *"`/Tabs` is inheritable through the page
...
//! "unspecified" or any other label. `D:\Dev\pdfce`'s own roadmap records
...
/// `pdfce_core::annot::MAX_ANNOTS_PER_PAGE`, plus one `HashMap` of widget ids
...
mod tests {
    ...
    fn node(tabs: Option<&[u8]>) -> Object {
        ...
        if let Some(t) = tabs {
            d.insert(
                pdfce_core::object::Name(b"Tabs".to_vec()),
                Object::Name(pdfce_core::object::Name(t.to_vec())),
            );
        }
        Object::Dict(d)
    }
    ...
    /// What that state mechanically denotes is recorded in `D:\Dev\pdfce`'s
    ...
    /// `pdfce-core` reads it without resolving through the graph, so a direct
    ...
    fn a_widget_with_no_p_entry_is_still_listed() {
        use pdfce_core::forms::{Field, FieldFlags, FieldType, FieldValue, Widget};
        ...
        w.insert(
            pdfce_core::object::Name(b"Rect".to_vec()),
            Object::Array(vec![
                Object::Integer(0),
                Object::Integer(0),
                Object::Integer(10),
    ...
    fn an_unclaimed_widget_and_a_plain_annotation_are_counted_apart() {
        ...
        let named = |k: &[u8], v: &[u8]| {
            let mut d = Dict::new();
            d.insert(
                pdfce_core::object::Name(k.to_vec()),
                Object::Name(pdfce_core::object::Name(v.to_vec())),
            );
            Object::Dict(d)
        };
        let graph = FakeGraph(HashMap::from([
