...
pub(super) fn rows(ui: &mut egui::Ui, doc: &OpenDoc, listing: &Listing, actions: &mut Vec<Action>) {
    ...
    let mut pressed: Option<(usize, pdfce_core::object::ObjId, Option<String>)> = None;
...
fn refusal_kind(error: &pdfce_core::edit::EditError) -> &'static str {
...
///
/// `pdfce_core::edit::EditError` is `#[non_exhaustive]`, so this needs a
/// wildcard whatever it does. The question is what the wildcard says, and the
/// answer is *"pdfce cannot, and this panel does not know why"* rather than a
/// plausible guess.
///
...
fn refusal_hint(error: &pdfce_core::edit::EditError) -> &'static str {
    use pdfce_core::edit::EditError as E;
    match error {
...
mod tests {
    ...
    /// gets one that says pdfce cannot and does not say why — because reaching
