...
//!
//! A button **already in the document** does not. `pdfce-core` could write an
...
//!    read as inert — pdfce asserting a fact about somebody else's document
...
//! `/SubmitForm` this reader does not yet decode is not *"an action pdfce will
//! not author"*, because pdfce authors submits happily, and calling it `Foreign`
...
//! decision"*, and greying would advertise a capability pdfce has chosen not to
//! have.
//!
//! ## Where the row is
//!
...
use egui::Ui;
use pdfce_core::edit::{ButtonAction, ButtonActionState, NamedAction, ResetScope};
use pdfce_core::forms::Field;
...
pub(super) fn row(
    ...
    // greying says "not now" and the truth is "not ever, by decision". pdfce
    ...
    if !open {
        ...
        if change.clicked() {
            ...
            // is knowable. `Unmodelled` cannot seed it — pdfce did not decode
...
mod tests {
    ...
    /// nothing"* about a button that resets the form — pdfce asserting a
    ...
    fn a_goto_seeds_the_box_one_based() {
        let does = from_core(&ButtonAction::GoToPage {
            page_index: 4,
            view: pdfce_core::edit::PageView::WholePage,
        });
