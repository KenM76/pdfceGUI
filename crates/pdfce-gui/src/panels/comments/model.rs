...
//!   `/RT` named that pdfce has never heard of,
...
//!
//! ## The ordering is `pdfce-cli list-annotations`', reused by name
//!
...
//! whatever [`pdfce_core::annot::page_annotations`] returns, which is the
//! array order the file itself carries — not a sort pdfce imposes.
//!
...
use pdfce_core::annot::{Annotation, Appearance, ReplyType, page_annotations};
...
impl Listing {
    ...
    /// document-wide "shapes pdfce drew carry no note" disclosure.
    ///
...
/// `pdfce_core::annot::MAX_ANNOTS_PER_PAGE` per page.
...
pub struct CommentRow {
    ...
    pub suppressed: bool,
    /// `Appearance::StateUnresolved` — pdfce could not select an appearance
...
/// (§14.9.3). Which one it is depends on the subtype, and `pdfce-core`
...
pub enum Note {
    ...
    /// shape pdfce itself drew: `MarkupSpec` has no contents field on any
...
pub enum Relation {
    ...
    /// core gives for keeping it: *"a name pdfce does not recognise is a
...
/// than a note.
///
/// The list is §12.5.6.2's, quoted in `pdfce-core`'s own docs: *"`Link` /
...
/// `pdfce_core::annot::Annotation` models **none** of those three: `/IT` is
...
/// properties — one pdfce authors and can restyle, regroup and delete as a
/// unit; the other is CAD-exported page content pdfce reads and must not
...
/// # Cost
///
/// One catalog → `/PieceInfo` → `/pdfce` → `/Private` walk and a
...
#[must_use]
pub fn ce_dimension_annots(session: &pdfce_core::edit::EditSession) -> BTreeSet<ObjId> {
    session
        .dimension_model()
        .dimensions()
        .iter()
...
mod tests {
    ...
    /// [`pdfce_core::annot::page_annotations`]' contract rather than this
    ...
    fn a_threaded_annotation_is_recognised_as_a_relation() {
        ...
        // reachable only from an `/RT` name pdfce has never seen.
        for r in related {
            assert!(matches!(
                r.relation,
    ...
    ///
    /// The condition the panel keys its "shapes pdfce drew carry no note"
