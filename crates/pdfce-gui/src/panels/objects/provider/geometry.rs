...
//! `crates/pdfce-gui/tests/leaf_geometry.rs`: a stub carries rectangles and
...
use egui::{Pos2, Rect};
use pdfce_core::vector::{Point, VectorObject};
...
impl ObjectModelProvider {
    ...
    pub fn subpath_hits_of(&self, target: TargetId, point: Pos2, tolerance: f64) -> Vec<usize> {
        ...
        pdfce_core::vector::hit::hit_test_subpaths_of(path, pdf, resolve(tolerance))
    }
    ...
    pub fn subpath_bounds_canvas_of(&self, target: TargetId, subpath: usize) -> Option<Rect> {
        ...
        let mut bounds: Option<pdfce_core::vector::Bounds> = None;
        for (_, p) in points {
            if !p.is_finite() {
                continue;
            }
            bounds = Some(match bounds {
                None => pdfce_core::vector::Bounds { min: p, max: p },
                Some(b) => pdfce_core::vector::Bounds {
                    min: Point {
                        x: b.min.x.min(p.x),
                        y: b.min.y.min(p.y),
                    },
