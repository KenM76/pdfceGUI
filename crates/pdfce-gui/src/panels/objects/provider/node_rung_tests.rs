...
//! space `vector::anchor_count` reports and `pdfce-cli node-move --node N`
//! addresses. A second numbering would make the number pdfce shows disagree
...
//! pdfce's old string-gate floor was test assertions, and a split that
