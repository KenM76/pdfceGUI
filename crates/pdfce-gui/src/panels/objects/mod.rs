...
//! position.** That is deliberate: `#n` is the number `pdfce-cli
...
//! `pdfce-core` already models and addresses with the same indices, so the
...
//! `pdfce_core::vector::PageObjects` has **no optional-content-group
...
pub(crate) mod test_support {
    ...
    ///
    /// `D:\Dev\pdfce\fixtures\synthetic\…`, reached by the same relative
    /// walk this crate's `Cargo.toml` uses to reach `pdfce-core` — so if the
    ...
    ///
    /// `D:\Dev\pdfce` is **read-only** for this project. These tests read
    /// from it and write nothing.
    pub fn engine_fixture(rel: &str) -> PathBuf {
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../../pdfce/fixtures/synthetic")
            .join(rel);
        assert!(
            path.exists(),
            "the engine fixture {rel} is missing at {}. This crate builds against \
             D:\\Dev\\pdfce by path, so if it compiled, the tree is there — check \
...
mod tests {
    ...
    use pdfce_render::tiny_skia::Transform;
