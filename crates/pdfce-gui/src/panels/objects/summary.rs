...
//! that is merely *described* imprecisely would be pdfce marking its own
...
//!
//! `pdfce_core::vector::TextObject` carries a decoded [`TextPreview`] and a
...
//! | `TextPreview::Decoded { lossy: true, .. }` | the string **plus** [`ObjectNote::TextPartlyUndecodable`] | The `\u{fffd}`s in the row are real; a note is what turns them from "pdfce is broken" into "this font's encoding is incomplete". |
...
//!    "dimension": **ce dimensions** are the ones pdfce authors, **pdf
...
impl ObjectSummary {
    ...
    /// pdfce is unsure about is content marking, and content marking is
...
mod tests {
    ...
    /// which would read as a pdfce defect rather than as a property of the
    /// file.
    #[test]
