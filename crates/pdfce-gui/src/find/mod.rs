...
//! character. **The old pdfce shell's Find bar ran through `find_text`**, so
//! typing a `?` into it matched every character on the page and nothing said
//! why. `pdfce-core` records that this was fixed *in the front end*, on
...
//! what moved was [`pdfce_core::edit::TextSearchOptions`]'s **default**, and
//! a front end that wants patterns now has to ask.
//!
//! So this module calls **[`pdfce_core::edit::EditSession::find_text_with`]
...
//! (`PdfceApp::open_path` and `PdfceApp::close_document`) — a hit list
...
//!
//! [`pdfce_core::edit::EditSession::find_text_with`] runs
...
//! walked, with fonts resolved. There is no cache in `pdfce-core` and none
...
//! run reports its own cost on the `PDFCE_DIAG` channel:
//!
//! ```text
//! pdfce-diag find needle="total" hits=47 current=1 page=3 ms=214 \
...
//!
//! ## Why the state lives on `PdfceApp` and not on `OpenDoc`
//!
//! `crate::app`'s rule: *state that dies with the document lives on
//! `OpenDoc`; state that outlives it lives on `PdfceApp`.* A find query and
...
impl Default for FindOptions {
    ...
    /// `pdfce-core` classifies it as **evidence tier (c)** — what other major
...
///
/// Not [`pdfce_core::edit::TextMatch`] itself, for two reasons that both
/// matter:
///
...
pub struct Hit {
    ...
    /// [`pdfce_core::annot_author::Quad`] in *unrotated PDF user space*
...
///
/// Lives on [`crate::app::PdfceApp`]; see this module's header for why the
...
pub struct FindState {
    ...
    /// document closed would be pdfce discarding a setting the operator made.
    options: FindOptions,
...
impl FindState {
    ...
    ///
    /// Called from `PdfceApp::open_path` and `PdfceApp::close_document`, the
...
///
/// Called from `PdfceApp::apply`, **after** the frame that raised it, which
/// is the only place the two borrows this needs can be had at once: the state
/// and the open document are separate fields of `PdfceApp`.
...
///
/// [`pdfce_core::edit::EditSession::find_text_with`] takes `&mut self` —
