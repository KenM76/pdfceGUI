...
//!
//! ```text
//! cargo test -p pdfce-gui --lib write_synthetic_image_only -- --ignored
//! cargo test -p pdfce-gui --lib recognises_the_synthetic_page -- --ignored --nocapture
//! ```
//!
...
//! exactly true of this repository: `D:\Dev\temp\pdfce\` holds thirty PDFs and
...
//!    `pdfce_core::ocr::layer::add_ocr_layer`;
//! 5. the invisible layer is written at text rendering mode **3**;
//! 6. and the result is a document whose text pdfce can extract — which is the
...
//!    ([`source_pdf`]);
//! 2. `pdfce_render` rasterizes it at [`FIXTURE_DPI`];
...
/// paragraphs below**, because it is a fact about the engine pdfce ships that
...
pub(crate) fn build() -> Vec<u8> {
    ...
    let rendered = pdfce_render::render_page(&doc, &pages[0], FIXTURE_DPI / 72.0)
...
mod tests {
    ...
    /// ★ **pdfce extracts nothing from it**, which is the condition the Find
    /// offer keys on.
    ///
    ...
    fn the_engine_finds_no_text_on_the_fixture() {
        let doc = pdfce_core::document::Document::from_bytes(build()).unwrap();
    ...
    ///
    /// ```text
    /// cargo test --release -p pdfce-gui --lib real_page_detection -- --ignored --nocapture
    /// ```
    #[test]
    ...
    fn real_page_detection() {
        ...
        let models = std::path::Path::new("D:/Dev/pdfce/crates/pdfce-core/assets/models/ocrs");
        let engine = pdfce_core::ocr::engine_ocrs::OcrsEngine::from_model_dir(models).unwrap();
        for (name, path) in [
            ("SW41177", "D:/Dev/temp/pdfce/SW41177.pdf"),
            (
                "a1-titleblock",
                "D:/Dev/pdfceGUI/fixtures/a1-titleblock.pdf",
            ),
        ] {
            let doc = pdfce_core::document::Document::load(std::path::Path::new(path)).unwrap();
            let pages = pdfce_core::page_tree::pages(&doc).unwrap();
            // Ground truth: the page's own vector text.
            let truth = pdfce_core::text_extract::extract_page_view(
                &doc.view(),
                &pages[0],
                0,
                &pdfce_core::text_extract::ExtractOptions::default(),
            ...
            for dpi in [72.0f32, 100.0, 150.0, 200.0, 300.0] {
                let r = pdfce_render::render_page(&doc, &pages[0], dpi / 72.0).unwrap();
    ...
    fn the_source_page_does_have_the_text_the_fixture_throws_away() {
        let doc = pdfce_core::document::Document::from_bytes(source_pdf()).unwrap();
        let text = pdfce_core::text_extract::extract_document_view(
            &doc.view(),
            &pdfce_core::text_extract::ExtractOptions::default(),
        )
    ...
    fn recognises_the_synthetic_page() {
        let models = PathBuf::from("D:/Dev/pdfce/crates/pdfce-core/assets/models/ocrs");
        assert!(
            models.is_dir(),
