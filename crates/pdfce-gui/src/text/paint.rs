...
//! swatch when pdfce cannot decode the ink.
//!
...
//! tells a drawing office what it needs; *"pdfce cannot show this colour"* tells
...
pub fn undecoded(ink: Option<String>) -> String {
    match ink {
        Some(name) => format!(
            "{name} — a named ink. pdfce will not overwrite it with a screen colour, because that \
             would look right here and change what prints."
        ),
        None => "Set in a colour space pdfce does not convert, so it is left exactly as it is."
            .to_owned(),
    }
}
...
/// where three are in a colour space pdfce will not rewrite needs to say 'nine
/// changed', not 'done'."*
#[must_use]
pub fn recoloured_partly(changed: usize, refused: usize) -> String {
    format!(
        "Recoloured {changed} object(s). {refused} were left alone — they are painted in inks \
         pdfce will not overwrite with a screen colour."
    )
}
