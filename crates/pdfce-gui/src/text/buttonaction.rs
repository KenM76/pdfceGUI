...
//!
//! `pdfce-core`'s reply on `set_button_action`, 2026-08-30, measured Acrobat
...
//! of Acrobat's dialog would be a **regression** against what pdfce can now
...
//! appears **zero times** in ISO 32000-1, so blocking `http://` would be pdfce
...
/// chooser reading *ResetForm / GoToPage / SubmitForm* would be pdfce showing
...
pub fn does_note(kind: ButtonDoesKind) -> String {
    match kind {
        ...
        ButtonDoesKind::Uri => {
            "Writes a web address into the document. pdfce never opens it; a reader program \
        ...
        ButtonDoesKind::SubmitForm => {
            "Writes an address into the document, and a declaration that the form's data \
             should be sent there. pdfce sends nothing and has no way to — but a reader \
...
/// and nothing in another. `pdfce-core` refuses one by name; this says why
...
/// 5. Not stated here because pdfce writes the baseline: `IncludeAppendSaves`
...
/// pdfce does not invent one, so this is the whole of the response: say it, and
/// let the operator decide. Refusing would be pdfce enforcing a rule nobody
...
///
/// ★★★ **Two commands, and the second one can fail on its own.** `pdfce-core`
...
mod tests {
    ...
    /// someone has made pdfce enforce a rule ISO 32000-1 does not state.
    #[test]
