...
//! not guessable from the refusal. The third names what pdfce takes, which is
...
pub fn only_the_first(count: usize) -> String {
    format!(
        "{count} files were dropped. pdfce opened the first one; drop the others one at a time."
    )
}
...
/// The file is not one pdfce takes.
///
...
pub fn not_accepted(ext: &str) -> String {
    if ext.is_empty() {
        "pdfce takes a PDF to open, or a PNG, JPEG, BMP or TIFF to place on the page. That file \
         has no extension, so pdfce could not tell what it was."
            .to_owned()
    } else {
        format!(
            "pdfce takes a PDF to open, or a PNG, JPEG, BMP or TIFF to place on the page. It \
