...
//! page content pdfce reads and must not silently alter. The distinction is
...
///
/// ★ The `NeverSet` arm renders `pdfce_core::dimension::NO_SCALE_DISCLOSURE`
...
pub fn scale_phrase(scale: ScaleState, unit: Unit) -> String {
    match scale {
        ScaleState::NeverSet => pdfce_core::dimension::NO_SCALE_DISCLOSURE.to_owned(),
...
///
/// ★ Deliberately does **not** claim conformance. `pdfce-core`'s own
/// `DimStandard` doc applies the same discipline — pdfce draws *ISO-style*,
...
pub const fn standard_hint() -> &'static str {
    "Sets the terminator form, whether the dimension line breaks for its text, \
     and how the extension lines are spaced. pdfce draws in the style of each \
...
pub const fn appearance_hint() -> &'static str {
    "These apply to every dimension in the group that has not been given its \
     own value. Clear one and pdfce's own default takes over again."
}
...
pub fn using_factory(value: &str) -> String {
    format!("using pdfce's default, {value}")
}
...
///
/// ★ **One vocabulary, shared with the CLI.** `pdfce-cli group-style` uses
...
mod tests {
    ...
    fn the_no_scale_disclosure_is_the_engines_own_words() {
        assert_eq!(
            ...
            pdfce_core::dimension::NO_SCALE_DISCLOSURE,
