...
//!
//! Every one of these sentences is a fact `pdfce-core` already computed and
...
use pdfce_core::vector::{FillRule, PaintStyle, Rgb, TextBoundsBasis};
...
pub fn objects_dock_empty_page_hint() -> &'static str {
    "This page has nothing pdfce can address individually — no shapes, text or images."
}

/// Empty state: the page's content could not be analysed.
#[must_use]
pub fn objects_dock_decompose_failed_hint() -> &'static str {
    "pdfce could not analyse this page's contents, so it cannot list its objects. The page may still display correctly."
}
...
/// useful property: `#412` is the number `pdfce-cli object-list` prints and
...
pub fn objects_dock_row_tooltip() -> &'static str {
    "The number is this object's position in the page's paint order — the same number pdfce's command-line tools use to address it."
}
...
/// pdfce shows and the number `pdfce-cli node-move --node N` addresses have
...
pub fn object_tree_node_tooltip() -> &'static str {
    "One anchor point of this part. The number counts across the whole object, so it matches what pdfce's command-line tools address."
}
...
/// rule" that is blank for nine rows in ten reads as a value pdfce failed to
...
/// `pdfce_core::vector::MAX_TEXT_PREVIEW_CHARS`, which is a memory budget
...
#[must_use]
pub fn font_label(font: &pdfce_core::vector::TextFont) -> String {
    let name = font
        .base_font
        .as_deref()
        .filter(|n| !n.is_empty())
...
pub fn object_note(note: ObjectNote) -> &'static str {
    match note {
        ObjectNote::ApproximateTextBounds(TextBoundsBasis::FontMetrics) => {
            "The area given for text is laid out from the font's own metrics: pdfce adds up the \
...
estimate: this font declares no ascent or descent for pdfce to read, so the area is one type \
...
no width table of its own, and is not one of the 14 standard faces whose metrics pdfce has \
...
not a box, which is why its height reads as 0.0 pt."
        ...
        ObjectNote::NoBounds => {
            "pdfce could not work out where this object is on the page, so it has no position or \
...
mod tests {
    use super::*;
    use pdfce_core::vector::TextFont;
    ...
    ///
    /// "This page really is blank" and "pdfce could not read this page" must
