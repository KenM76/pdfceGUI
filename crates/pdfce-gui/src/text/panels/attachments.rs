...
//!
//! Each one exists because `pdfce-core` states an obligation in its own doc
...
//! | [`name_was_changed`] | `sanitize_attachment_name`: the name in a document is attacker-controlled and unconstrained — `..\..\Windows\System32\evil.exe`, `invoice.pdf\0.exe`, `CON.txt` are all authorable — so pdfce writes a different file name than the row shows, and *"pdfce renamed this file"* with no reason is the sneaky behaviour rule 4 forbids. |
//!
...
//! non-conforming — `pdfce-core` records this as ambiguity **EF-A2** and its
//! own `DeclaredSizeCheck::Disagrees` doc says to word it *"the document says
//! 999999 and pdfce counted 10"*, not *"this document is broken"*. [`size`]
...
//! design: `pdfce-core` has no shared §7.9.4 date type yet and says outright
...
//!
//! [`DeclaredSizeCheck::is_contradicted`]: pdfce_core::attachments::DeclaredSizeCheck::is_contradicted

use pdfce_core::attachments::{AttachmentNotes, DeclaredSizeCheck, NameHazard};
...
/// ordinary; an operator reading this must not be left wondering whether pdfce
/// failed to look.
#[must_use]
...
///
/// The distinction this states is the one `pdfce_core::attachments`' module
...
/// measurement** — `pdfce-core` does not sniff the bytes, and `/text#2Fplain`
...
///
/// | state | what pdfce actually knows |
/// |---|---|
...
/// | `Agrees` / `Disagrees` | pdfce counted. Only here is a comparison honest. |
///
...
pub fn size(declared: Option<u64>, check: DeclaredSizeCheck) -> String {
    match check {
        ...
        // difference between this arm and every other one below: pdfce counted
        ...
        DeclaredSizeCheck::Disagrees { declared, actual } => format!(
            "The document says {} and pdfce counted {}.",
            human_bytes(declared),
            human_bytes(actual)
        ),
        DeclaredSizeCheck::Unverified => match declared {
            Some(bytes) => format!(
                "{} as declared — compressed, so pdfce has not checked it yet.",
                human_bytes(bytes)
            ),
...
/// neither.
///
/// Printed verbatim. See the module header: `pdfce-core` stores these raw
...
pub const fn date_tooltip() -> &'static str {
    "Shown exactly as the file wrote it. pdfce does not reformat a date it has not parsed."
}

// ---------------------------------------------------------------------------
// What pdfce could not vouch for — the per-row disclosures
// ---------------------------------------------------------------------------

/// The name shown is pdfce's best reading of bytes it could not fully decode.
///
...
/// after a UTF-16BE BOM, an unpaired surrogate. That is **pdfce's own
...
pub const fn no_bytes() -> &'static str {
    "This entry points at a file kept outside the PDF, so pdfce has nothing to save out."
}
...
pub const fn may_be_encrypted() -> &'static str {
    "This document is encrypted, so anything saved out of it may be unreadable — pdfce does not decrypt attachments yet."
}
...
/// *"there are no page attachments"* and *"pdfce could not go and look"* — and
...
pub fn listing_notes(notes: &AttachmentNotes) -> Vec<String> {
    ...
    if notes.truncated {
        said.push(
            "This document has more attachments than pdfce lists; the rest are not shown."
                .to_owned(),
        );
    }
    if notes.name_tree_budget_exhausted {
        said.push(
            "pdfce stopped walking this document's attachment index early — it is deeper or larger than pdfce follows, so some entries are missing from this list."
                .to_owned(),
        );
    }
    if notes.name_tree_cycles > 0 {
        said.push(
            "This document's attachment index loops back on itself. pdfce skipped the loop; the file is malformed."
                .to_owned(),
        );
    }
    ...
    if notes.page_tree_unwalkable {
        said.push(
            "pdfce could not read this document's page tree, so it did not look for files attached to pages. The list above is complete only for the document itself."
                .to_owned(),
        );
    }
    said
...
/// `pdfce-core` has no verb that edits one afterwards, so an operator who
...
pub const fn attach_description_note() -> &'static str {
    "A note about what this file is. It can only be set now — pdfce cannot edit a description afterwards."
}
...
///
/// It is worded as a limit of pdfce rather than as a fault in the file, because
...
pub const fn attach_refused_multi_node_tree() -> &'static str {
    "pdfce cannot add to this document's attachment index — it is stored in a form pdfce would risk damaging. The files already attached are unharmed and can still be saved out."
}
...
pub fn attach_source_unreadable(detail: &str) -> String {
    format!("pdfce could not read that file, so nothing was attached: {detail}")
}
...
/// still fully present on disk."* Saying what pdfce does is how an operator can
...
/// makes the absence legible rather than mysterious. It is not that pdfce
...
/// the word suggests.
///
/// # This sentence is required by `pdfce-core`, in its own words
///
...
/// inside a file that arrived from somewhere, and `pdfce-core`'s own module
/// docs say it *"does not execute, open, or interpret them, and neither should
/// a caller without its own gate."* pdfce writes the file and stops; opening it
...
pub const fn save_tooltip() -> &'static str {
    "Write this file to disk. pdfce does not open or check what is in it — the type shown is only what the document claims."
}
...
/// ★★★ **pdfce used a different name than the row shows, and here is why.**
///
...
/// completes. pdfce reports the **raw** name in the listing, because *"a
...
/// suspicious. And pdfce refuses to *use* that raw name on a filesystem,
...
pub fn name_was_changed(from: &str, to: &str, hazards: &[NameHazard]) -> String {
    let mut said = format!("The document calls this file {from}; pdfce saved it as {to}");
...
pub fn extract_failed(detail: &str) -> String {
    format!("pdfce could not read that attachment out of the document: {detail}")
}

/// The file could not be written.
#[must_use]
pub fn save_failed(detail: &str) -> String {
    format!("pdfce could not write that file: {detail}")
}
...
mod tests {
    ...
    /// `AttachmentNotes` is `#[non_exhaustive]`, so no crate but `pdfce-core`
    ...
    fn a_damaged_listing_speaks_and_its_sentences_are_distinct() {
        ...
        let (_, notes) = pdfce_core::attachments::list_attachments_with_notes(&doc);
    ...
    /// that would catch a flag pdfce sets over-eagerly: a `Default` is a value
    /// nobody produced, and a listing that quietly reported *"pdfce stopped
    ...
    fn a_well_formed_document_needs_no_caveat() {
        ...
        let doc = pdfce_core::document::Document::load(&path).expect("the fixture loads");
        let (found, notes) = pdfce_core::attachments::list_attachments_with_notes(&doc);
