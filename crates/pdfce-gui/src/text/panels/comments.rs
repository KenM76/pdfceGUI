...
//!
//! - [`comments_all_without_notes`] exists because pdfce's own markup
//!   authoring cannot write `/Contents` on a geometric shape — `MarkupSpec`
//!   has no contents field on any variant, deliberately
//!   (`D:\Dev\pdfce\crates\pdfce-core\src\annot_author.rs:210-212`) — so a
//!   document whose annotations pdfce drew shows a column of identical "no
...
//! | [`comment_row_appearance_unresolved`] | §3.4.4 — pdfce *"displays nothing and does not guess"*, and the governing default is explicitly a reasoned guess, i.e. an inference, i.e. rule 4 applies |
...
//!
//! *"Never write a bare 'dimension".* **ce dimensions** are the ones pdfce
...
/// records why: pdfce's own markup tools cannot attach a note to a shape
/// (`MarkupSpec` has no contents field on any variant, and that is
/// deliberate), so a document whose annotations pdfce authored shows a column
...
/// the three this project owes pdfce, per `HANDOFF.md` §1. The day it lands,
...
/// `pdfce_core::annot::Annotation::subtype_label`, which is the `/Subtype`
...
///
/// Project rule 15: a **ce dimension** is the thing pdfce authors — a `/Line`
...
/// (`D:\Dev\pdfce\crates\pdfce-core\src\annot.rs:340-347`). Printing an
...
/// `pdfce-core` stores it raw and its own docs say *"do not assume it
...
/// only wanted by whoever wonders why pdfce did not tidy it.
#[must_use]
pub fn comment_row_modified_tooltip() -> &'static str {
    "Shown exactly as the file wrote it. The standard lets this be a date or any text at all, and requires a reader to accept whatever is there, so pdfce does not reformat it."
}
...
/// would be putting pdfce's voice inside a quotation.
#[must_use]
...
/// this state on a document pdfce drew on.
///
...
/// A row whose appearance state pdfce could not resolve.
///
...
/// means pdfce *"displays nothing and does not guess a first / `On` / `Off`
...
/// guess**. That makes the blank page an *inference of pdfce's*, and rule 4
...
pub fn comment_row_appearance_unresolved() -> &'static str {
    "pdfce could not work out which appearance this annotation should use, so it draws nothing for it. That is pdfce declining to guess, not a fault in the page."
}
...
///
/// # The one row in this panel where pdfce and another reader will disagree
///
/// §12.5.6.2 says a group subordinate's own `Contents`, `M`, `C`, `T`,
/// `Popup`, `CreationDate`, `Subj` and `Open` *"shall be ignored"* in favour
/// of the group primary's. `pdfce-core` **deliberately does not apply that
...
/// `pdfce-core`'s read half must never do"* (`annot.rs:326-335`).
///
...
/// Saying so is rule 4's *"pdfce inferred something, and another reader may
...
pub fn comment_row_is_group_member() -> &'static str {
    "Part of a group. The standard says a reader should show the group's main annotation's note here instead; pdfce shows what this one actually says, so another viewer may show something different."
}
...
mod tests {
    ...
    ///
    /// *"**ce dimensions** are the ones pdfce authors; **pdf dimensions** are
    /// CAD-exported page content pdfce reads and must not silently alter. They
    ...
    /// accept any format, so `pdfce-core` stores it raw. A catalog entry that
    ...
    /// fact rather than pdfce's own edit.
    #[test]
    ...
    /// pdfce itself drew — `MarkupSpec` carries no contents field, deliberately
    /// — so this caption is the *ordinary* case on a pdfce-marked document, and
    ...
    /// decorated them would put pdfce's voice inside a quotation.
    #[test]
