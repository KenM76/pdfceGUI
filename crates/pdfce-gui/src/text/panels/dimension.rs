...
//!
//! The **bottom tier** of `pdfce-core`'s style cascade, made reachable. Eleven
...
//!
//! A **ce dimension** is one pdfce authors; a **pdf dimension** is CAD-exported
//! page content pdfce reads and must not alter. This catalog says *"this
...
//!
//! `pdfce-cli dimension-style` names these eleven `unit`, `fraction`,
...
//! `pdfce` a shipped defect: *"A panel that previews 'nominal + tolerance' by
...
use pdfce_core::dimension::{DecimalMarker, DimStandard, StyleSource, Tolerance, Unit};
...
/// from another file, a merge, an earlier pdfce that wrote the annotation and
...
pub const fn no_record() -> &'static str {
    "This looks like a dimension pdfce authored, but this document carries no \
...
///
/// ★ **`Factory` is rendered as *"pdfce's default"* and NOT as *"inherited"***
...
pub const fn source_name(source: StyleSource) -> &'static str {
    match source {
        StyleSource::Factory => "using pdfce's default",
...
///
/// `pdfce-core`'s own `FractionMode` doc calls the unreduced form *"the
...
///
/// `pdfce-core`'s `tolerance.rs` states it as a property of the model — the
...
/// negative (write ±0.1, not ±-0.1)"* — and `pdfce-core`'s own comment on the
...
/// the digit count. Naming it is the whole reason pdfce spells it as an absent
/// value.
#[must_use]
