...
//!    the fourteen makes pdfce **write a new object into the operator's file**.
...
/// picking one writes to their file; *"pdfce can add"* says what the click does
...
///
/// `pdfce-core`'s own release note for `Pass 162.0`: pdfce *"authors the font
...
pub const fn face_addable_disclosure() -> &'static str {
    "Choosing one of these adds it to the document. pdfce writes the face's name and its letter \
...
mod tests {
    ...
    ///    *are*.
    /// 2. **Not embedded** — the fact `pdfce-core` states and this shell relays.
    ...
    /// identically — `Helvetica` the page carries and `Helvetica` pdfce would
    ...
    fn the_two_group_headings_say_different_things() {
        ...
        // The addable heading must read as an act pdfce performs. "Standard
