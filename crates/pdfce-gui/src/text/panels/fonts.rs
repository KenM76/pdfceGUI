...
//! push a mutation through [`pdfce_core::edit::EditSession`], and at stage
...
//!
//! - A blocked verdict is a fact about the **file**, not a pdfce failure, so
...
/// place pdfce did not look" is part of the answer rather than a hedge on
/// it. A list that quietly missed a surface and looked complete is this
/// project's most-repeated defect shape.
///
/// Acrobat's own coverage here is recorded as an unconfirmed GAP, so pdfce
...
/// rejection notice.
///
/// It states the count and nothing more. Naming a remedy pdfce cannot
...
// than its bits say. Saying "the licence permits" would be pdfce making a
...
// absent case, so pdfce defines none either.
// ---------------------------------------------------------------------------
...
/// version 3 onward, and pdfce reports the ambiguity rather than resolving
/// it.
#[must_use]
        ...
        for s in [
            ...
            assert!(
                ...
                "an fsType sentence that states a permission without attributing \
                 it to the vendor's bits is pdfce making a legal claim: {s}"
            );
        }
        // The raw value is always printed, so an operator can check pdfce's
