...
//!
//! Because the alternative shipped once and was a defect. `pdfce-core`'s
//! [`pdfce_core::edit::TextSearchOptions::wildcards`] records it in full:
...
//! [`pdfce_core::edit::WordBoundary`] carries the whole argument. What
...
pub fn field_tooltip() -> &'static str {
    "Type what to look for, then press Enter. Enter again goes to the next hit and \
     ...
     pdfce searches the text drawn on the pages — not form fields, comments, bookmarks \
     or attachments."
}
...
/// `pdfce-core`'s documented limits on `find_text`, not this shell's.
#[must_use]
pub fn no_matches_tooltip() -> &'static str {
    "Nothing on any page matched. pdfce searches only text that is drawn with real glyphs, \
...
pub fn wildcards_tooltip() -> &'static str {
    "Off by default, so pdfce searches for exactly what you typed. Switch it on and \"#\" \
...
/// hedging: `pdfce-core`'s `WordBoundary` quotes §14.8.2.5 NOTE 1 saying the
