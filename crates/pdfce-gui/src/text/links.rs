...
//!
//! Because it navigates. `pdfce_core::outline::Destination::Page` is the only
...
//! | `NonNavigation` | `/URI`, `/Launch`, `/JavaScript`, `/SubmitForm` | nothing here; pdfce does not perform these |
//!
...
///
/// ★ pdfce does not open it, and the sentence says which file rather than
...
/// so that it cannot be read as a failure. pdfce's standing rule is that it
...
pub fn non_navigation(action: &str) -> String {
    format!(
        "This link is a {action} action, not a page jump. pdfce shows what it is and does not run it."
    )
}
...
/// every page index in `pdfce-core` is, and every page number this program
