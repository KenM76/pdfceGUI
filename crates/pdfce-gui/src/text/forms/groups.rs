...
/// case (a two- or three-field group, which is what `pdfce`'s own dotted-path
...
pub fn field_group_preview_refused(name: &str) -> String {
    format!(
        "pdfce could not work out what deleting \u{201c}{name}\u{201d} would remove, so \
...
pub fn field_group_delete_refused(name: &str) -> String {
    format!(
        "\u{201c}{name}\u{201d} was not deleted \u{2014} pdfce declined the change and the form \
