...
//! operator has drawn — the ones pdfce authors. It is *not* read from **pdf
//! dimensions**, the CAD-exported page content pdfce reads and must not alter,
...
//! is told pdfce has no evidence rather than being given a number.
//!
//! ## ★ The one sentence this whole window exists for
//!
//! `pdfce-core`'s own doc on `DxfOptions::scale`:
//!
...
//! | `Uncalibrated` | nothing on this page carries a scale | says pdfce has no evidence, and that 1:1 is a *choice* rather than a finding |
...
use pdfce_core::export::dxf::{DxfOutcome, DxfUnits};
...
/// ★ pdfce inferred the scale, and this says **where from**.
///
...
///
/// Not refused, and not resolved by pdfce picking one. A sheet holding a 1:50
...
/// A DXF unit's name.
///
/// ★ pdfce writes only inches and millimetres, and `DxfUnits::for_unit` maps
...
pub const fn no_geometry() -> &'static str {
    "pdfce has not read this page's geometry yet, or could not. Nothing was \
     exported."
}
...
/// `pdfce-core` states the case in its own field doc:
///
...
/// - **unreadable** — pdfce *could not read it*: no font resolver in scope, or
...
mod tests {
    ...
    ///
    /// The assertion this catalog exists for. `pdfce-core` names the failure it
    ...
    ///
    /// `skipped` is *you asked*; `unreadable` is *pdfce could not read it*. The
    ...
    /// pdfce will change, and the second reads as a gap somebody might fix.
    #[test]
    fn a_skipped_picture_names_the_formats_limit_not_pdfces() {
        let note = exported(
            "a.dxf",
            &DxfOutcome {
                polylines: 1,
