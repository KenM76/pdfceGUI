...
//!   reads as pdfce failing to find something that is plainly on the page.
...
///
/// The obvious implementation counts `pdfce_core::forms::Field::is_fillable`,
...
/// whole document, and when a field holds rich text pdfce cannot author.
///
...
///
/// `pdfce_core::forms::AcroForm::inline_field_roots` counts `/Fields` entries
...
/// the whole truth. An operator comparing pdfce's "9 fields" against another
...
pub fn forms_inline_field_roots_note(count: usize) -> String {
    format!(
        "⚠ {count} more entry(ies) in this form are written in a way the PDF standard does not \
         allow — as values rather than as references — so they have no identity pdfce can \
...
///
/// **pdfce never runs a document's JavaScript**, and that is a standing
...
pub fn forms_javascript_note(count: usize) -> String {
    format!(
        "⚠ {count} field(s) carry scripts that would normally calculate, format or validate \
         their value. pdfce does not run them, so a value you type here stays exactly as typed \
...
/// property of the FILE (`pdfce_core::forms::AcroForm::xfa`), knowable before
...
pub fn forms_xfa_note() -> &'static str {
    "⚠ This form also carries an XFA packet, which describes the same fields a second time. \
     pdfce fills the part most viewers read and cannot write the XFA part, so an XFA-aware \
...
pub fn form_field_certification_disabled_tooltip() -> &'static str {
    "A certification signature on this document forbids changing form values. Filling this \
     field would invalidate that signature, so pdfce will not do it."
}

/// Note on a signature-field row.
///
/// Listed rather than hidden (R83): an operator scrolling past a signature
/// field should see that pdfce knows it is there.
#[must_use]
...
/// (`edit.rs:12607-12629`). `pdfce_core::forms::Widget::on_states` lists the
/// states the widget's `/AP` `/N` sub-dictionary defines, **excluding `Off`**
/// — so an empty list means there is no state pdfce may select, and a tick
...
pub fn form_field_no_on_state_note() -> &'static str {
    "This document records no ticked state for this box — it defines the drawn appearance for \
     only one of the two. pdfce will not invent one, so the box can be read here but not \
     changed."
}
...
/// fidelity: pdfce cannot author `/RV`, and §12.7.3.4 / §12.7.3.3 bind
...
pub fn form_field_rich_text_note() -> &'static str {
    "This field holds formatted text. pdfce cannot edit that formatting yet, and typing plain \
...
pub fn form_field_rich_text_not_utf8() -> String {
    "This field holds formatted text that pdfce could not read — the stored formatting is not \
...
pub fn form_field_rich_text_unreadable(reason: &str) -> String {
    format!(
        "This field holds formatted text that pdfce could not read ({reason}). It is NOT \
...
pub const fn recompute_explainer() -> &'static str {
    "pdfce never runs a document's JavaScript. Where a field is computed by a recognised \
     Acrobat built-in, pdfce can reproduce the arithmetic natively instead. The source script \
...
/// of the inputs pdfce had to read as zero — are read together.
#[must_use]
...
///
/// `reason` is `pdfce_core::form_script::recompute::Skip`'s own `Display`,
...
/// it is wrong here. `pdfce-core` has no batch-recompute verb — part 3 of the
...
pub const fn recompute_apply_tooltip() -> &'static str {
    "Writes the values listed above — one undo step per field, because pdfce writes them one \
...
///
/// A rule-4 disclosure of the purest kind: pdfce inferred something (which
...
pub fn recompute_order_is_a_guess(unlisted: usize) -> String {
    format!(
        "This form does not list {unlisted} of its calculated field(s) in its calculation \
         order, which the PDF standard requires. pdfce ordered them by their own dependencies; \
...
pub fn recompute_not_considered(count: usize) -> String {
    format!(
        "{count} other script(s) were not considered — pdfce recognises no built-in in them, so \
...
/// `pdfce_core::forms::Field::has_appearance` reports per field — has nothing
...
mod tests {
    ...
    /// of an inert control is *"pdfce got it wrong"*. Two that read alike
    ...
    ///
    /// "pdfce never runs a document's JavaScript" is a project rule, and the
