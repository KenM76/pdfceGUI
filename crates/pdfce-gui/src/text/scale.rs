...
//!
//! What this window calibrates is a group of **ce dimensions** — the ones pdfce
//! authors — and never a **pdf dimension**, which is CAD-exported page content
//! pdfce reads and must not silently alter. The copy avoids the bare word and
...
use pdfce_core::dimension::{FractionMode, Unit};
...
///
/// `pdfce_core::dimension::Unit` is **not** `#[non_exhaustive]`, unlike most
...
/// pdfce has never heard of. The two are opposite situations and get opposite
...
/// the useful half is which one pdfce could not use.
#[must_use]
...
pub fn parse_failed(detail: &str) -> String {
    format!("pdfce could not read that length: {detail}")
}
