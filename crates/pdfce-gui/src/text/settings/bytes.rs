//! # `text::settings::bytes` — what changing it makes pdfce WRITE
//!
...
pub const fn separations_refuse_note() -> &'static str {
    "pdfce declines rather than splitting a set of plates apart, and tells you \
...
/// appearance with no indication pdfce had chosen it.
#[must_use]
pub const fn missing_as_nothing_note() -> &'static str {
    "Refuses to guess. Nothing appears, and pdfce counts it so you can see how \
...
/// out the shipped one was *"arguably wrong on pdfce's own invariant"* — and
/// it was: objects pdfce did not logically touch are re-emitted byte-identical,
...
pub const fn xref_eol_match_note() -> &'static str {
    "Saving a document pdfce did not otherwise change leaves its index untouched. \
...
pub const fn xref_eol_space_lf_note() -> &'static str {
    "What pdfce wrote for every file before it learned to match. Choose a fixed \
