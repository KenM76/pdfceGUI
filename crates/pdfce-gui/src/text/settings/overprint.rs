...
//! rather than a line count: every other block in `look` is about how pdfce
...
//! underneath it showing. The argument is over *which* colours count, and pdfce
//! has changed its mind once, for a good reason:
//!
//! * Until 2026-09-03 the default was `GreyAsKOnly` — a deliberate divergence
//!   from the standard, recorded as such, kept because pdfce flattened spot
...
///
/// "(pdfce's default)" was hard-coded onto `GreyAsKOnly` from the day this
...
///
/// The label went on saying "(pdfce's default)" about the option that was no
...
pub fn zero_tint_default_suffix(
    ...
    if scope == pdfce_core::settings::OverprintZeroTintScope::default() {
        "  (pdfce's default)"
    } else {
        ""
    }
...
pub const fn zero_tint_note(scope: pdfce_core::settings::OverprintZeroTintScope) -> &'static str {
    ...
    match scope {
        S::GreyAsKOnly => {
            "A grey fill is treated as black ink alone. This is what pdfce did before September 2026, and what Acrobat does. Choose it if a page changed appearance in this build and you want the old rendering back."
        }
        S::DeviceCmykOnly => {
...
mod tests {
    ...
    ///
    /// This is the tripwire for the defect that produced it. "(pdfce's
    ...
    /// option pdfce had stopped using was the one it used. The engine's own
    ...
    /// would leave two options both claiming to be what pdfce does.
    #[test]
