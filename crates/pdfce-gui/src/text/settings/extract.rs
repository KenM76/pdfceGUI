...
pub const fn word_gap_note() -> &'static str {
    "Measured as a fraction of the text size. Raise it if words are being split \
     apart; lower it if separate words are running together. pdfce's default of \
...
pub const fn unmappable_replacement_note() -> &'static str {
    "Puts a visible marker where the text could not be read, so the gap is \
     ...
     that both keeps the length right and looks wrong, which is pdfce's own \
...
/// `pdfce-core` and was shown nowhere: the layout pass drops a run with no
...
pub const fn actual_text_always_note() -> &'static str {
    "The author said what this text really is, so pdfce believes them, and \
     ...
     places, so this is pdfce reading the stronger of the two."
}
...
///
/// New in this port; the old window disclosed it nowhere despite `pdfce-core`
...
pub const fn actual_text_bound() -> &'static str {
    "Whichever you choose: where a document supplies replacement text, pdfce can \
