...
///
/// `SpotColorantDeviceModel`, new in `pdfce-core 0.20`. The two values are
...
pub const fn spot_model_label(
    ...
    use pdfce_core::settings::SpotColorantDeviceModel as M;
    match model {
        M::SimulateSeparations => "Keep the ink on its own plate (pdfce's default)",
...
#[must_use]
pub const fn spot_model_note(model: pdfce_core::settings::SpotColorantDeviceModel) -> &'static str {
    use pdfce_core::settings::SpotColorantDeviceModel as M;
    match model {
