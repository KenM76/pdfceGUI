...
//!
//! `pdfce-core` reports the answer as a number rather than as a warning, and
...
/// distinguishes *"the file said 300 dpi"* from *"pdfce assumed 72"*, and the
...
pub fn natural_size(width_mm: f64, height_mm: f64, declared_dpi: Option<(f64, f64)>) -> String {
    match declared_dpi {
        ...
        None => format!(
            "{width_mm:.0} × {height_mm:.0} mm — the file declares no resolution, so pdfce \
...
pub const fn fit_name(fit: ImageFit) -> &'static str {
    match fit {
        ...
        // mode pdfce gains renders as the engine's own debug name rather than
...
/// ★ **Why pdfce re-encoded the picture instead of storing the file's bytes.**
///
/// `RecompressReason` carries no `Display`, and that absence is a decision
/// rather than an omission: these are *pdfce's* reasons, in pdfce's vocabulary
...
/// exhaustive and can never fail to compile when pdfce grows a sixth reason —
...
pub const fn recompress_reason(reason: RecompressReason) -> &'static str {
    match reason {
        ...
        RecompressReason::NoCompressedSource => {
            "the file was not compressed, so pdfce compressed it"
        }
...
/// messages **name the operator's file** — *"pdfce does not place GIF images —
/// it places PNG, JPEG, BMP and TIFF"*, *"this image uses {feature}, which
/// pdfce cannot place"* — so the specific half is the whole value and a
...
pub fn placement_disclosures(
    ...
    if let Some(reason) = recompressed {
        out.push(format!(
            "pdfce re-encoded the picture rather than storing the file's own \
             bytes — {}. {}",
...
mod tests {
    ...
    ///
    /// The number is always given because `pdfce-core` insists it is *"not a
    ...
    fn an_assumed_resolution_says_it_was_assumed() {
        ...
        assert!(
            ...
            "and says what pdfce did instead: {assumed}"
        );
    }
}
