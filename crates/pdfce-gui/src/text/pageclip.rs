...
/// what pdfce is holding really is a document. The wording says so, because an
...
pub fn os_marker(pages: usize) -> String {
    if pages == 1 {
        "1 page copied from pdfce. Paste it back into pdfce, or use Paste in the Pages tab."
            .to_owned()
    } else {
        format!(
            "{pages} pages copied from pdfce. Paste them back into pdfce, or use Paste in the \
             Pages tab."
        )
    }
}
