...
/// prose pdfce made up about somebody else's addition.
#[must_use]
...
/// stamps bake a name and a timestamp into the appearance; pdfce has no
...
/// somebody would be a claim pdfce cannot support — and saying so is cheaper
/// than an operator looking for the feature.
#[must_use]
pub const fn stamp_bound() -> &'static str {
    "These are the standard stamps. pdfce does not add a name or a date to \
