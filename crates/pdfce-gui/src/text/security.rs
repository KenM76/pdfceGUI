...
//!
//! ## ★★★ Two sentences here came from `pdfce-core` and must not be re-worded
//!
...
//! instructive: our version said pdfce *"cannot tell you the document is
...
use pdfce_core::crypto::{AuthKind, Cipher, PermissionBit};
...
/// pdfce is reading a local file and rate-limiting the operator's own guesses
...
/// The **different** failure: pdfce cannot normalise a non-ASCII password.
///
/// # ★★★ Why this is not "wrong password", and why the engine made it a
/// separate error
///
/// `DocError::PasswordRequiresNormalisation` exists, in `pdfce-core`'s own
...
/// before hashing, and pdfce does not implement it — no stringprep dependency
...
/// perfectly correct**, and pdfce cannot prove it either way. Sending them to
/// re-type it would be this program wasting their afternoon on its own
/// limitation.
#[must_use]
pub const fn password_needs_normalisation() -> &'static str {
    "That password contains characters outside plain ASCII, and pdfce cannot process those the way this document's encryption requires — so it cannot open the file even if the password is correct. This is a limit in pdfce, not a wrong password. A document whose password is plain ASCII will open normally."
}

/// The refusal for an empty box.
///
/// ★ Refused here rather than sent on, and the reason is in
/// [`crate::secret::Secret::is_empty`]: pdfce has *already* tried the empty
...
pub const fn password_empty() -> &'static str {
    "Type the password first. pdfce already tried opening this document without one."
}
...
pub fn cipher_line(cipher: Cipher) -> String {
    let described = match cipher {
        ...
        // ★ `/None` means the security handler decrypts privately and pdfce
        ...
        Cipher::None => {
            "a handler pdfce does not implement — the document is marked encrypted and its content is not protected in any way pdfce can see"
        }
    };
...
///
/// Supplied by `pdfce-core` on 2026-09-03 with the instruction *"take this one
...
/// it**.
///
/// `pdfce-core` reports it and prefers neither value, keeping `/P` because that
...
/// header.
///
/// Our draft said pdfce *"cannot tell you the document is unaltered"*. The
...
pub const fn signature_not_verified() -> &'static str {
    "pdfce can see that this document is signed and can tell you whether anything was appended after the signature. It does not yet check the signature itself — so it cannot tell you the signed bytes are unaltered, and it cannot tell you who signed it or whether to trust them."
}
...
/// The coverage verdict — the one signature fact pdfce *can* state today.
///
...
pub const fn cannot_author() -> &'static str {
    "pdfce can read a document's protection and cannot change it: it cannot add or remove a password, change these permissions, or sign a document. Those are being built in the engine — encryption first, signing later."
}
