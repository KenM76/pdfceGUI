...
pub const fn intro() -> &'static str {
    "Every key pdfce responds to, read from the same table that dispatches \
...
pub const fn no_keymap() -> &'static str {
    "pdfce could not read its own shortcut table, so no keys are bound in this \
...
mod tests {
    ...
    ///
    /// *"pdfce could not read its table"* and *"nothing is bound"* are a fault
