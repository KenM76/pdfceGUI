...
//! and `pdfce-core` is explicit that it is **choosing** this disclosure rather
...
use pdfce_core::font_embed_missing::EmbedBlocker;
...
/// possible, and the catch-all says *"pdfce would not embed it"* rather than
...
pub fn blocked_row(face: &str, blocker: &EmbedBlocker) -> String {
    let why = match blocker {
        ...
        EmbedBlocker::ProgramUnrecognised => {
            "the file found for it is not a font pdfce can read".to_owned()
        }
        EmbedBlocker::ProgramIsCollection => {
            "the file found for it holds several faces in one, which pdfce cannot embed from"
                .to_owned()
        }
        _ => "pdfce would not embed it, and this build cannot say why".to_owned(),
    };
    format!("{face} — {why}")
}
...
///
/// ★★ pdfce is **choosing** this disclosure rather than matching Acrobat —
...
pub fn pdfa_line(claim: &PdfaClaim) -> Option<String> {
    match claim {
        ...
        PdfaClaim::Identified { part, conformance } => {
            ...
            Some(format!(
                "This document says it is {named}. Embedding is usually what a PDF/A needs — an \
                 ...
                 the document's, and pdfce does not re-check it."
            ))
        }
        ...
        // this keeps it.
        _ => Some(
            "This document's metadata could not be read, so pdfce cannot say whether it claims \
             to be PDF/A."
                .to_owned(),
        ),
    }
...
pub fn refused(detail: &str) -> String {
    format!("pdfce would not embed into this document: {detail}")
}
...
/// prediction would make pdfce wrong on every single embed; reporting it as a
