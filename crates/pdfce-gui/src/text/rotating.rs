...
//! stroking, and no scalar `/BS /W` describes an anisotropic stroke, so pdfce
...
//! > and step (c) concatenates that with the placement matrix. So pdfce
...
//!
//! **Name the thing the operator can see, never the thing pdfce models.**
...
impl RotateRefusal {
    ...
    pub const fn line(self) -> &'static str {
        match self {
            ...
            Self::WrongVerb => {
                "pdfce cannot turn that kind of item, and it changed nothing rather than turn part of it. Form fields and dimensions each need their own tool."
            }
            // ★★ "Signed", not "certified": the operator's word for what
            // happened to the file is that somebody signed it. And it says the
            // limit is the DOCUMENT's rather than pdfce's, because an operator
            // told only "cannot" will look for a setting to change.
            Self::Certified => {
                "This document has been signed, and the signature does not allow it to be changed this way. pdfce turned nothing."
            }
            ...
            Self::Other => {
                "pdfce could not turn that, and it changed nothing — the page is exactly as it was, and there is nothing to undo."
            }
        }
    }
}
...
/// `pdfce-core` flagged it before this shell had drawn a single rotated shape:
///
