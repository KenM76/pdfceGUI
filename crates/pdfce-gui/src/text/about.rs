...
//!
//! pdfceGUI shipped only permissively-licensed *code*. MIT and Apache-2.0
...
//! check. Nobody was handed a file whose licence obliged pdfce to *tell them*
...
//! for someone who was handed `pdfce-gui.exe` in a folder.
//!
//! Auditing what pdfce already ships, in the course of building the surface
...
//! through the engine (`pdfce-core` and `pdfce-render` are path dependencies
//! and Rust links them statically), so they are inside `pdfce-gui.exe`
...
//! | this dialog | the **attribution**: who made it, what it is, under what terms, and whether pdfce changed it | someone who runs the program |
//!
...
//! in a folder is invisible to an operator who launched pdfce from a shortcut
...
/// One third-party work `pdfce-gui.exe` redistributes, and everything an
...
pub struct Attribution {
    ...
    /// Whether pdfce changed the work, stated plainly either way.
    ///
...
/// written `pdfce` everywhere, and a title-cased About box would be the one
...
/// operator with two pdfce builds installed nothing they did not know. Names
/// the engine explicitly: the single most common question about this binary
/// is whether it contains pdfce or merely talks to it.
#[must_use]
pub fn summary() -> &'static str {
    "A PDF viewer and editor. The pdfce engine is built into this program; nothing else is required to run it."
}

/// pdfce's own licence, and its copyright line.
///
...
pub fn full_texts_note() -> &'static str {
    "Full licence texts for these, and for every Rust crate pdfce links, are in THIRD_PARTY_LICENSES.md in the folder beside the program."
}
...
///   nobody re-reads it.
/// - **The pdfce icon set** (`crates/pdfce-gui/src/icons/assets/`). It is the
...
/// a verbatim work in a collection, and pdfce's own licence is unaffected —
...
pub fn attributions() -> &'static [Attribution] {
    &[
        // Source: D:\Dev\pdfce\crates\pdfce-render\assets\fonts\PROVENANCE.md
        // and the "Bundled Foxit substitute faces" section of this
        // repository's `about.hbs`, which reproduces the pdfium LICENSE in
        // full. Redistributed because `pdfce-render`'s `font::bundled` embeds
        // all fourteen faces with `include_bytes!`, so they are inside this
        // executable.
        //
        // Covers the asset directory `crates/pdfce-render/assets/fonts` in
        ...
        // for it: crates/pdfce-render/assets/fonts
        Attribution {
            component: "The 14 substitute font faces pdfce draws with when a document embeds no font",
        ...
        // `about.hbs`.
        Attribution {
            component: "The glyph-name to Unicode mapping pdfce uses to extract text",
            creator: "Adobe Systems Incorporated",
        ...
        // Source: D:\Dev\pdfce\crates\pdfce-core\assets\models\ocrs\PROVENANCE.md
        ...
        //
        // Covers the asset directory `crates/pdfce-core/assets/models/ocrs` in
        ...
        // crates/pdfce-core/assets/models/ocrs
        Attribution {
            component: "The two neural-network weight files pdfce recognises text with",
...
mod tests {
    ...
    /// pdfce's own licence line agrees with the file that ships beside it.
    ///
