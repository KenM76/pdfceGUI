...
//! ## Rule 15
//!
//! What these tools author is a **ce dimension** — one pdfce writes. A **pdf
//! dimension** is CAD-exported page content pdfce reads and must not alter.
...
//!
//! `pdfce-core` gives the two-line gesture three things a shell is expected to
...
use pdfce_core::dimension::TwoLineRefusal;
...
/// 2. **A virtual apex.** The lines meet only if extended. `pdfce-core` is
...
/// re-exported from `pdfce-core`, so a `TwoLineAuthoring` cannot be built in a
...
mod tests {
    ...
    /// the decision a decision — `pdfce-core`'s own words, and the reason it
