...
//!
//! **Unembedding does not make the file smaller when pdfce saves it.**
//!
...
//! soften the number — it is to **state the number and then state that pdfce's
...
use pdfce_core::font_unembed::{UnembedBlocker, UnembedPlan};
...
pub const fn nothing_removable() -> &'static str {
    "There is no embedded font in this document that pdfce can safely remove."
}
...
/// `crate::app::save` writes incrementally, so pdfce's own Save leaves every
...
pub fn size_note(bytes: u64) -> String {
    format!(
        "This frees {} of font data. **pdfce's Save will not make the file smaller**, because it \
         ...
         needs a save that rewrites the whole file, which pdfce does not offer yet.",
        bytes_phrase(bytes)
    )
}
...
/// — and `pdfce-core` deliberately does **not** refuse on it, saying in as many
...
pub fn pdfa_line(claim: &PdfaClaim) -> Option<String> {
    match claim {
        ...
        _ => Some(
            "This document's metadata could not be read, so pdfce cannot say whether it claims to \
...
mod tests {
    ...
    /// ★★★ **The size sentence always says pdfce's Save will not deliver it.**
    ///
