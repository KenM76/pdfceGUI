...
//! seriously without being alarming**, because pdfce runs none of these and
...
//! pdfce recognises actions and never executes one — so the operator learns
//! that pdfce cries wolf and stops reading the status row. That costs every
...
///
/// Because it changes what every other clause means. *"pdfce could not finish
...
pub fn disclosure(reach: ReachOut) -> String {
    ...
    if parts.is_empty() {
        return "pdfce could not finish checking this document for submit, launch and script \
                actions, so treat it as unchecked rather than clean."
            .to_owned();
    }

    let what = join_and(&parts);
    // ★★★ The clause that carries the tone. It is the reassurance (pdfce will
    ...
    let body = format!(
        "This document can {what}. pdfce never does any of that \u{2014} it reads these and \
         leaves them alone \u{2014} but a viewer that runs them would."
    );

    if reach.truncated {
        format!(
            "pdfce could not finish checking this document, so there may be more than this. {body}"
        )
    } else {
        body
    }
...
mod tests {
    ...
    /// ★★★ Every sentence says pdfce does not do it AND that something else might.
    ///
    ...
    fn every_finding_carries_both_halves_of_the_tone() {
        for r in [
            ...
            let s = disclosure(r);
            assert!(
                s.contains("pdfce never does any of that"),
