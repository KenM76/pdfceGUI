...
//! operation pdfce has — unembedding a font, deleting a page, deleting an image
...
//!
//! Because incremental saving is not a limitation pdfce is working around — it
...
/// understands *"pdfce normally adds to the end of the file"* can predict what a
/// rewrite costs; one told only *"this makes your file smaller"* cannot.
#[must_use]
pub const fn intro() -> &'static str {
    "pdfce normally saves by adding your changes to the end of the file, which keeps the earlier \
...
pub const fn revisions_line() -> &'static str {
    "The earlier version of the drawing that pdfce keeps inside the file is dropped. Your own \
...
/// from every other loss pdfce discloses. A signature covers a byte range
...
///
/// ★★ A real refusal with a named cause, not a fallback. `pdfce-core` refuses a
...
pub fn refused(detail: &str) -> String {
    format!("pdfce cannot rewrite this file: {detail}. Save a copy the ordinary way instead.")
}
...
pub fn write_failed(detail: &str) -> String {
    format!("pdfce could not write the compacted copy: {detail}")
}
