...
//! distinction `pdfce-core`'s `signature` module draws, into operator English,
...
//! until it cannot.
//!
//! Read `D:\Dev\pdfce\crates\pdfce-core\src\signature.rs`'s module
...
//! reason the second wording is careful in a way the first is not. pdfce
...
//! under-reporting is pdfce making a silent claim about the integrity of a
...
//!    otherwise read pdfce's silence as a pass.
...
///
/// ★★ It asserts the outcome flatly, because here pdfce can. §12.8.1 makes a
...
/// The engine works pdfce's operations against that table and concludes that
...
///
/// The verdict here is pdfce's cautious one rather than the standard's: for an
...
/// pdfce cannot source in the largest type in the window, which is exactly
...
/// [`basis_approval`], one line below, gives pdfce's verdict together with the
/// fact that it is pdfce's.
#[must_use]
...
/// the certifier's list, its closedness, and the finding that no pdfce
/// operation is on it.
///
...
///
/// 1. **Report pdfce's verdict**, which is `Invalidated`. Not reporting it
...
/// plainly: pdfce would rather over-report a reviewable hint than make a
/// silent claim about a legal artifact. An operator reading it learns both
/// what pdfce thinks and how much weight to give it, which is more than either
/// half alone would tell them.
#[must_use]
pub const fn basis_approval() -> &'static str {
    "The PDF standard does not settle whether that invalidates a signature of this kind. pdfce \
...
/// carries the whole of what pdfce can honestly put on a button here: there is
/// something to weigh, the operator has read it, and they are proceeding. It
/// does not assert an outcome pdfce cannot source.
///
...
/// The footnote under the buttons: pdfce has not checked anything.
///
...
/// available when an operator wonders what pdfce actually knows.
///
...
/// dictionary. Without this sentence, an operator who sees pdfce speak
/// confidently about a signature here will reasonably conclude that pdfce's
...
pub const fn verifies_nothing() -> &'static str {
    "pdfce does not check any signature's certificate or its cryptography. It reports what a \
...
pub fn preserved_note(count: usize) -> String {
    ...
    format!(
        "{subject}, and this save was appended, so the bytes each signature covers are \
         ...
         changes are ones the signer allowed is a separate question, and pdfce does not answer \
         it."
    )
}

/// The status-bar note after a save that pdfce reports as invalidating.
///
...
///
/// ★ It says *"pdfce reports"* rather than *"this invalidated"*, and it says
...
/// verdict to pdfce is true when the basis is a spec citation (pdfce is
...
pub fn invalidated_note(count: usize) -> String {
    ...
    format!(
        "{subject}, and the save you just made changes it after signing. pdfce reports that as \
...
mod tests {
    ...
    fn the_preserved_note_pairs_the_fact_with_the_uncertainty() {
        for count in [1, 3] {
            ...
            assert!(
                note.contains("pdfce does not answer it"),
    ...
    fn the_two_footings_are_worded_differently() {
        ...
        assert!(
            ...
            "ISO 32000-1 is silent for a plain approval signature; the headline must not put \
             pdfce's cautious verdict in the largest type as though it were the standard's"
        );
    }
    ...
    /// the standard is silent would be pdfce citing a clause that does not
    /// exist — and it would read as more authoritative than the certified
    /// case, which is exactly backwards.
    #[test]
    fn the_approval_footing_attributes_the_verdict_to_pdfce() {
    ...
    /// ★★ **Nothing here says a signature is valid, or that pdfce checked
    /// one.**
    ///
    /// `pdfce-core`'s signature module opens *"This module verifies nothing."*
    ...
    /// the thing pdfce has not established, plus *verified* and *we checked*,
    ...
    fn no_string_claims_a_signature_is_valid_or_was_verified() {
        ...
        for s in [
            ...
            for forbidden in [
                ...
                assert!(
                    !lower.contains(forbidden),
                    "`{forbidden}` asserts something pdfce has not established: {s}"
                );
            }
        }
    }
