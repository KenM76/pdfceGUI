...
This note follows the format of `D:\Dev\pdfce`'s own asset provenance records
(`crates/pdfce-render/assets/fonts/PROVENANCE.md`,
`crates/pdfce-core/assets/models/ocrs/PROVENANCE.md`) so that a reader moving
...
- **Changes made by pdfce:** files copied from ScripTree are byte-identical
  apart from being **renamed** to pdfce's *role* rather than ScripTree's shape
  name (`icon-folder.svg` → `folder.svg`, and so on). Glyphs with no ScripTree
  ancestor were drawn for pdfce.
...
folder and already covers this art. Adding a section that reproduced pdfce's
...
`crates/pdfce-gui/src/icons/assets.rs` §1–§4 — it is long, it is the thing a
