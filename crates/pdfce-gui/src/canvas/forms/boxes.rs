...
use pdfce_core::forms::{AcroForm, ButtonKind, Field, FieldFlags, FieldType, FieldValue, Widget};
...
/// small: `pdfce-core`'s corpus has nothing over a thousand fields.
///
...
/// correction: the first version of this walk read `pdfce_core::forms::Widget::page`
...
/// conformant and is common in the wild, and `pdfce-core` additionally reads
...
/// fixtures in `D:\Dev\pdfce\fixtures\synthetic\forms\` write `/P` on every
...
mod tests {
    ...
    ///
    /// ```text
    /// cargo test -p pdfce-gui a_drawn_text_field_fixture -- --ignored --nocapture
    /// ```
    ///
    /// # Why this has to exist
    ///
    /// **Not one of the eleven form fixtures in
    /// `D:\Dev\pdfce\fixtures\synthetic\forms\` carries a text field with a
    ...
    fn a_drawn_text_field_fixture() {
        ...
        let out = std::env::temp_dir().join("pdfce-drawn-text-form.pdf");
        ...
        let rotated = std::env::temp_dir().join("pdfce-drawn-text-form-rotated.pdf");
        ...
        let view = session.view();
        let form = pdfce_core::forms::parse_acroform(&view).expect("the form survived");
        let field = form
            .fields
            .iter()
