...
//! detail. pdfce is a CAD-drawing tool: `pdfce-core` carries a whole
...
//! calls [`pdfce_core::dimension::format_measurement`] — the function that
...
//!
//! [`Scale::of`] reads [`pdfce_core::edit::EditSession::dimension_model`] and
//! takes the **default group** (`DEFAULT_GROUP_ID`), which `pdfce-core`
//! guarantees always exists. That is the document's own
//! `/PieceInfo /pdfce /Private` sidecar, so a sheet calibrated in a previous
//! session — by this application once the scale surface lands, or by
//! `pdfce-cli` today — is honoured on open with nothing else wired.
//!
...
//! which is every test document in `D:\Dev\temp\pdfce`. That is why this is
//! written as *reading a document property* rather than as *reading a pdfce
...
//! `dimension_model()` walks catalog → `/PieceInfo` → `/pdfce` → `/Private`
...
//!
//! pdfce draws the second, and the reason is what a grid is *for*. A drafter
...
//! smaller fit scale on frame N+1 is a different label on frame N+2. pdfce has
...
//! forbids pdfce drawing *its own inferences* onto the page — a badge saying
...
//! pdfce found** — a detected drawing frame, an inferred module size. There is
...
use pdfce_core::dimension::{DEFAULT_GROUP_ID, NumberFormat, ScaleState, Unit, format_measurement};
...
/// format.
///
/// See this module's header, §1. `Copy`, and both fields are `pdfce-core`
...
impl Default for Scale {
    ...
    /// `pdfce-core` has no `Unit::Point`, because points are what a
...
impl Scale {
    ...
    /// be rendered.
    ///
    /// Straight through [`pdfce_core::dimension::format_measurement`] — see
