...
//! about marking up what has been read, because Acrobat is what pdfce
...
//!    amplitude — approximating an appearance stream `pdfce-core` generates. Two
...
//! appearance stream. [`spec`] hands `pdfce-core` a `MarkupSpec` and
//! `EditSession::add_markup` does the rest, which is the same route
//! `pdfce-cli`'s `markup-add` takes with the same value — the equivalence the
...
use pdfce_core::annot_author::{Color, MarkupSpec, Quad, TextMarkupKind};
...
pub enum TextMarkKind {
    ...
    ///
    /// pdfce authors this natively **even though Acrobat's own UI does not** —
    /// a deliberate exceed-Acrobat choice recorded in `pdfce-core`'s
...
impl TextMarkKind {
    ...
    /// The `pdfce-core` subtype this kind authors.
    ///
...
/// Build the `pdfce-core` spec one text-markup command authors.
///
