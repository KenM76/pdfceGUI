...
//!   the operator never draws the closing segment, the file does. **pdfce is in
...
//! subtype's, applied by `pdfce-core`'s `polygon_like(…, closed: true)`. So the
...
const VERTEX_MEMORY_KEY: &str = "pdfce-markup-vertex-run";
...
///
/// Published by `crate::app::PdfceApp::conditions` and read by `markup.finish`'s
