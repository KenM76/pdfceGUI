...
//!
//! This project told `pdfce-core` that a resize must **not** scale stroke
...
//! whether to hold Shift. A settings dialog is where you say what pdfce should
...
//!
//! ⇒ pdfce refuses instead, **by name**, and this switch is the operator's way
...
//! reference does."*
//!
//! ★ It applies only where pdfce did **not** author the appearance. An
//! appearance pdfce built is rebuilt from the scaled geometry at the new size,
...
///
/// Every field maps one-to-one onto a `pdfce_core::edit::ResizeOptions` field,
...
const KEY: &str = "pdfce-resize-modifiers"; // ui-text-exempt: internal memory id, never displayed
...
impl Modifiers {
    ...
    #[must_use]
    pub fn to_options(self) -> pdfce_core::edit::ResizeOptions {
        // ★ Builders, not a struct literal: `ResizeOptions` is
        // `#[non_exhaustive]`, so the struct form — including
        // `..Default::default()` — is a compile error outside `pdfce-core`, and
