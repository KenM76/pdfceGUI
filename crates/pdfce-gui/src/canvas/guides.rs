...
//! a guide pdfce placed *itself*, on a margin or a frame it detected. There is
...
const DRAG_KEY: &str = "pdfce-canvas-guide-drag"; // ui-text-exempt: internal memory id, never displayed

/// `egui::Id` base for the guides' catch bands.
const BAND_KEY: &str = "pdfce-canvas-guide-band"; // ui-text-exempt: internal widget id, never displayed
...
/// The path this store reads and writes, or `None` when `pdfce-core` found no
/// writable location.
///
/// Derived from the same `pdfce_core::settings::resolve_store()` call that
...
/// `pdfce_core::settings::store_in`, and it exists for the same two reasons:
...
mod tests {
    ...
    fn guides_survive_a_reopen_and_stay_with_their_own_document() {
        let dir = std::env::temp_dir().join(format!("pdfce-guides-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
    ...
    ///
    /// Not hypothetical: `D:\Dev\temp\pdfce` sits under a user profile on this
    ...
    fn a_missing_store_answers_no_guides() {
        let missing = std::env::temp_dir().join("pdfce-guides-does-not-exist-4471.txt");
