...
//! > by `pdfce-gui` (`grep Overlays crates/pdfce-gui/src` → zero hits), so
...
//! is a pdfce concept wearing a colour. `egui_shell::theme::Overlays` is
...
//! `tools/gates/check-shell-purity.sh` scopes to `pdfce-*` crate and module
...
//! |---|---|---|
//! | `preview` | `notice` | *"Something is worth knowing and nothing is broken"* — which is precisely what a snap marker is. It is a **proposal**: pdfce saying *here is where I think you are pointing*, before any click has committed anything |
