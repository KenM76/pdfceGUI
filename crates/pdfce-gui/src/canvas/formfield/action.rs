...
//!
//! The shell's model of `pdfce_core::edit::ButtonAction`, and the one place it
...
//!
//! Absent, permanently, and not as an omission: `pdfce-core` refuses both by
...
//! and a greyed *Run a script* would advertise a capability pdfce has decided
//! not to have.

use pdfce_core::edit::{ButtonAction, NamedAction, PageView, ResetScope};
...
pub enum ButtonDoesKind {
    ...
    ShowHide,
    /// §7.11 — open a web address. pdfce never follows one.
    Uri,
...
pub struct ButtonDoes {
    ...
    /// hides nothing, depending on the reader. `pdfce-core` refuses one by name
    ...
    /// therefore out of pdfce's scope rather than merely unbuilt.
    pub hide: bool,
...
///
/// A shell mirror of `pdfce_core::edit::PageView`, for the same reason
...
pub enum ActionBlocker {
    ...
    UrlMissing,
    /// An address `pdfce-core` will refuse because it cannot state it
    ...
    pub fn to_core(&self) -> Option<ButtonAction> {
        ...
        match self.kind {
            ...
            ButtonDoesKind::SubmitForm => {
                ...
                let spec = pdfce_core::edit::SubmitSpec::new(self.url.trim());
...
/// Whether an address is one pdfce can state unambiguously.
///
...
/// Blocking it would be pdfce inventing a conformance requirement. It is
...
/// states no TLS rule, and pdfce does not invent one. What it does is say so.
#[must_use]
...
mod tests {
    ...
    /// pdfce enforce a rule the standard does not state — see [`url_blocker`].
    #[test]
