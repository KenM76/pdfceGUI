...
//! | [`TextAnnotKind::Stamp`] | **drag a rectangle** | pdfce's stamp appearance is a framed label scaled into its rect, so the drag is choosing how big the stamp is |
//!
...
/// on every stamp regardless of what it said, so a reader other than pdfce
...
mod tests {
    ...
    /// and any reader but pdfce would show *Draft*. An annotation that
