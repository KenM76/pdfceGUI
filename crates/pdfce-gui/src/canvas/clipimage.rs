...
//! a bitmap cannot express — and it is the right payload for pdfce→pdfce work
...
pub fn publish(clip: &ObjectClip, text: &str) -> Option<(u32, u32)> {
    let pdf = clip.to_pdf();
    let doc = pdfce_core::document::Document::from_bytes(pdf.bytes).ok()?;
...
///
/// Because that is `tiny_skia`'s contract and therefore `pdfce-render`'s: its
