...
const FRAME_MEMORY_KEY: &str = "pdfce-canvas-frame"; // ui-text-exempt: internal memory id, never displayed

/// `egui::Memory` key for the one-shot marquee-zoom arming flag.
const REGION_MEMORY_KEY: &str = "pdfce-canvas-zoom-region"; // ui-text-exempt: internal memory id, never displayed

/// `egui::Memory` key for "a pending anchor has already waited one frame".
const WAITED_MEMORY_KEY: &str = "pdfce-canvas-anchor-waited"; // ui-text-exempt: internal memory id, never displayed
...
/// Report a framing zoom on the `PDFCE_DIAG` channel and hand the outcome
/// back.
///
