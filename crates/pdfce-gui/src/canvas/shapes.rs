...
//! that goes through `pdfce-render` is a second away.
//!
//! **This does not go through `pdfce-render`.** `vector::decompose_page` has
...
/// `ObjectModelProvider::object_node_points` reports and `pdfce-cli node-move`
/// speaks.
///
...
/// so the preview is right about pdfce even where it would be wrong about
...
fn screen(p: Point, page: &pdfce_core::page_tree::Page, map: &PageMapping) -> Option<Pos2> {
