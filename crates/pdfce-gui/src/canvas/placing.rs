...
const PLACING_MEMORY_KEY: &str = "pdfce-canvas-placing"; // ui-text-exempt: internal memory id, never displayed

/// Where a completed placement waits for `app::frame` to collect it.
const RESULT_MEMORY_KEY: &str = "pdfce-canvas-placing-result"; // ui-text-exempt: internal memory id, never displayed
...
const CANCELLED_MEMORY_KEY: &str = "pdfce-canvas-placing-cancelled"; // ui-text-exempt: internal memory id, never displayed
...
/// ago.
pub fn take_result(ctx: &egui::Context) -> Option<(PlaceKind, pdfce_core::page_tree::Rect)> {
...
/// `1224 − 759 = 465`.
pub fn click(ctx: &egui::Context, page: &pdfce_core::page_tree::Page, point: egui::Pos2) {
    let Some(pending) = pending(ctx) else {
        return;
    };
...
fn finish(ctx: &egui::Context, kind: PlaceKind, rect: pdfce_core::page_tree::Rect) {
    ctx.data_mut(|d| {
        d.remove::<Pending>(id(PLACING_MEMORY_KEY));
...
/// `pdfce_core::page_tree::Rect` is not `Clone` in the way `egui::Memory`
...
struct PlacedRect(pdfce_core::page_tree::Rect);
...
mod tests {
    ...
    /// `rotate`.
    fn test_page(w: f64, h: f64) -> pdfce_core::page_tree::Page {
        pdfce_core::page_tree::Page {
            id: pdfce_core::object::ObjId::new(1, 0),
            resources: pdfce_core::object::Dict::new(),
            media_box: pdfce_core::page_tree::Rect::from_corners(0.0, 0.0, w, h),
            crop_box: pdfce_core::page_tree::Rect::from_corners(0.0, 0.0, w, h),
            rotate: 0,
            contents: Vec::new(),
