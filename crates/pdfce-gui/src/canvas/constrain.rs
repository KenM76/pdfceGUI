...
//! and the old pdfce shell all answer Shift the same way. An operator who holds
//! Shift and gets a free-form resize does not conclude that pdfce chose
...
//!
//! It travels through `egui::Memory` rather than a field on `PdfceApp`, for the
...
fn slot() -> egui::Id {
    egui::Id::new("pdfce.canvas.constraint") // ui-text-exempt: memory key, never displayed.
}
