...
//! [`pdfce_core::text_edit::EditOptions`](pdfce_core::text_edit::EditOptions) —
...
//! own doc comment (`pdfce-core/src/text_edit/edit.rs`, the `FollowerDisposition`
//! enum):
//!
...
//! (`pdfce-core/src/text_edit/edit.rs`, inside `plan_edit`):
//!
//! ```text
//! Rec::Tm([a, b, c, d, e, f]) => {
...
//! signal that `pdfce-core` does not publish, and inventing one here would be
//! the shell deciding a question the engine owns.

use pdfce_core::text_edit::{AlignmentSource, BlockAlignment, DetectedAlignment};
use pdfce_core::text_edit::{EditOptions, FollowerDisposition};
...
/// `pdfce-core` can build one, so a [`choose`] that took it could only ever be
...
///
/// **Ported, not chosen.** It is `pdfce-core`'s own `MTX_EPS` from
...
/// [`GlyphProvenance`](pdfce_core::text_extract::GlyphProvenance) publishes;
...
pub fn choose(
    ...
    // a `match` from outside `pdfce-core` must carry one. It answers
