...
//! ```
//!
//! ★★ **The reassembly is `pdfce-core`'s and always was.**
...
/// move to the next block of text"*, and it is `pdfce-core`'s recognition doing
/// the work.
///
...
fn byte_offset(
    text: &pdfce_core::text_extract::PageText,
    run: usize,
    caret: usize,
) -> Option<usize> {
...
/// (`pdfce-core`'s reading order), so a lone label in the middle of a drawing
...
mod tests {
    ...
    ///
    /// Everything else here is `pdfce-core`'s recognition, which has its own
