...
//!
//! ```text
//! cargo test -p pdfce-gui --lib canvas::textedit::cost -- --ignored --nocapture
//! ```
//!
//! ## What is being measured, and why it is not a micro-benchmark
//!
//! There is **no public dry-run** in `pdfce-core`. `plan_edit` — the function
...
//!    from the loop entirely. **This is a feature request for `pdfce-core`**, and
...
use pdfce_core::text_extract::{ExtractOptions, extract_page_view};
...
fn corpus() -> Vec<(&'static str, PathBuf)> {
    ...
    for (label, p) in [
        ...
        (
            ...
            r"D:\Dev\pdfceGUI\fixtures\a1-titleblock.pdf",
        ),
    ] {
...
fn measure(label: &str, path: &Path) {
    ...
    let plan_ms = find.as_ref().map_or(f64::NAN, |f| {
        ...
        if pdfce_core::text_edit::edit_text(&doc, &req, &EditOptions::default()).is_err() {
            return f64::NAN;
        }
        timed(5, || {
            let _ = pdfce_core::text_edit::edit_text(&doc, &req, &EditOptions::default());
        })
    });
