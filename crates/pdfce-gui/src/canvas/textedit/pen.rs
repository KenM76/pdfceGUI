...
//!
//! `canvas::markup::pen::Pen` is a field on `PdfceApp`, and `panels::tool`'s
...
//!   which is a font *file* the operator has to choose and pdfce has to subset.
...
use pdfce_core::text_edit::addtext::NewTextFace;

/// The `egui::Memory` key the text pen is parked under.
const KEY: &str = "pdfce.textedit.pen"; // ui-text-exempt: memory key, never displayed
...
/// `0 g` is one operator and one byte where `0 0 0 rg` is four, and pdfce's
