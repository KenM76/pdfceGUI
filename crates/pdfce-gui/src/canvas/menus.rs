...
//! `PdfceApp::dispatch_token`, the same choke point the ribbon's Delete
...
const MENU_MEMORY_KEY: &str = "pdfce-canvas-menu"; // ui-text-exempt: internal memory id, never displayed
...
///    which `PdfceApp::new` treats as "the ribbon does not render and the
...
/// 3. **The conditions, corrected.** `PdfceApp::conditions()` was evaluated
