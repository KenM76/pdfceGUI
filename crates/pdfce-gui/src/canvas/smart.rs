...
/// no new reason.
const ENABLED_KEY: &str = "pdfce.smart-select.enabled"; // ui-text-exempt: a memory key, never displayed

/// The container the pointer is currently working inside. Memory key.
const ENTERED_KEY: &str = "pdfce.smart-select.entered"; // ui-text-exempt: a memory key, never displayed
