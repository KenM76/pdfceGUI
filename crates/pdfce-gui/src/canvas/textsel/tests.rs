...
fn on_string(ctx: &PageContext<'_>, word: &str, fraction: f32) -> Pos2 {
    ...
    let glyph = |g: &pdfce_core::text_edit::GlyphRef| {
        ctx.text
            .runs
            .get(g.run)
...
///
/// Run it deliberately:
/// `cargo test -p pdfce-gui --lib the_operators_own_vertical_stamp -- --ignored --nocapture`
///
...
fn the_operators_own_vertical_stamp_comes_back_whole() {
    ...
    let doc = pdfce_core::document::Document::load(path).expect("the drawing loads");
