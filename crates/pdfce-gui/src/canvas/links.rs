...
//!
//! A link's destination **could not be read**. `pdfce_core::annot::Annotation`
...
//! page, a lost name table, another file, and an action pdfce deliberately does
...
use egui::{Pos2, Rect};
use pdfce_core::outline::{Destination, RemoteTarget};
...
pub fn follow(hit: &Hit, doc: &OpenDoc, actions: &mut Vec<Action>) {
    ...
    match &hit.destination {
        ...
        // says the link exists and pdfce does not know what it is, which is
