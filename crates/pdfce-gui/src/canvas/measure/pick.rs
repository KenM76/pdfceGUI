...
//! `pdfce-cli` (`c7c1744`); the old GUI shipped the "Measure ▾" menu, the three
...
//! fit-set, all expressed over `pdfce-core` types (never egui), so every
...
//!   same engine path).
//!
//! The scale half of that list — [`pdfce_core::dimension::preview_group_scale`]
//! and [`pdfce_core::dimension::parse_length`] — is stated in full on
