...
//!
//! The **fit** — [`pdfce_core::dimension::fit_circle_taubin`] — and the
//! **authored value**, `pdfce-core`'s own `DimensionKind`. Nothing here
...
use pdfce_core::dimension::{DimensionKind, FitCircle, fit_circle_taubin};
...
/// [`fit_circle_taubin`] consumes and the frame `pdfce-cli dimension-add
...
/// (`SW41177.pdf`, page 1, measured with `pdfce-cli object-list`) three objects
...
impl CircularPick {
    ...
    ///
    /// The same point set `pdfce-cli dimension-add --points` would pass, so the
    ...
    /// inference pdfce cannot make is not made on the operator's behalf.
