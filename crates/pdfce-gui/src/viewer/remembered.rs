...
//! `egui-shell`, not of `pdfce-gui`, and `Cargo.toml` is not this work's to
//! edit.
//!
...
//! would cost a field on `PdfceApp`, a load at start-up, and a staleness
//! question ("what if another pdfce window wrote it?") that not caching
...
/// and for the identical reason: `pdfce-gui drawing.pdf` gives a relative
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("pdfce-gui-display-{tag}-{nanos}"));
    ...
    /// The file sits in the directory `pdfce-core` resolves, beside the other
