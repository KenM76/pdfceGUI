...
//! `layout.ron`**, and the directory is resolved by asking `pdfce-core`,
...
//! dependencies of `egui-shell`, not of `pdfce-gui`, and `Cargo.toml` is not
...
//! `egui-shell` would teach the reusable shell about a pdfce feature, which
...
//!    [`crate::app::PdfceApp::ribbon_band`]'s custom-item renderer, which
...
///
/// Held by [`crate::app::PdfceApp`] for the whole session. Cheap to
/// construct once — [`Self::load`] performs `pdfce-core`'s writability probe
...
impl RecentFiles {
    /// Load the list from the directory `pdfce-core` puts settings in.
    ///
    ...
    ///
    /// The twin of `pdfce_core::settings::store_in` and of
    ...
    ///
    /// `pdfce-gui drawing.pdf` gives `argv[1] = "drawing.pdf"`, and a
    ...
    /// [`crate::app::PdfceApp::dispatch_command`].
...
///
/// Called from [`crate::app::PdfceApp`]'s custom-item renderer for the
...
/// `PdfceApp::dispatch_command` — the same choke point a ribbon click, a
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("pdfce-gui-recent-{tag}-{nanos}"));
    ...
    /// `pdfce-core` and to nothing else.
    #[test]
    ...
    ///
    /// `Default` exists so `PdfceApp` keeps deriving it, and because
    /// `PdfceApp::new` deliberately uses it under `cfg(test)` so a unit test
