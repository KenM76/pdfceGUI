...
//! `pdfce-core` has no verb that changes one afterwards, so there is no
...
use pdfce_core::attachments::{self, Attachment, AttachmentKind};
...
pub enum AttachmentAction {
    ...
    /// this route). `pdfce-core` exposes **no verb that edits one afterwards**,
    ...
    SaveCopy {
        ...
        /// says whether pdfce had to use a different one on disk.
        name: String,
    },
}
...
/// operator picked and pdfce cannot read must decline **before** the session is
...
/// literal here, so the fallback pdfce *writes* and the fallback pdfce
...
mod tests {
    ...
    fn the_suggested_path_is_the_document_s_folder_and_the_attachment_s_name() {
        ...
        let document = pdfce_core::document::Document::load(&path).expect("the fixture loads");
        let pages = pdfce_core::page_tree::pages(&document).expect("a page tree");
        let open = OpenDoc::new(path, pdfce_core::edit::EditSession::new(document), pages);
