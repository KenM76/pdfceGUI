...
//!
//! A **ce dimension** is one *pdfce itself authors*: a `/Line` annotation
...
//! pdfce reads and must not silently alter. Nothing in this file touches the
...
/// 3. **It matches what the engine did.** `pdfce-core` groups these verbs
///    behind one module (`pdfce_core::dimension`) and one sidecar, for the
///    same reason.
///
...
pub enum DimensionAction {
    ...
    /// `pdfce-cli dimension-add` builds** from the same picks, so a ce
    ...
    ///
    /// `pdfce-core`'s own reasoning, quoted because it is the answer to the
    ...
    /// panel click and a `pdfce-cli group-style` invocation the same edit.
    SetGroupStyle {
...
mod tests {
    use super::*;
    use pdfce_core::dimension::DEFAULT_GROUP_ID;
