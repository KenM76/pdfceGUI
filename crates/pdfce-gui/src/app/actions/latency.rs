...
//! |---|---|---|
//! | **(a) the commit** | `EditSession`'s verb — locate, plan, rewrite the content stream, stage the objects | `pdfce-core` |
//! | **(b) the raster** | redrawing the page at the current zoom because the edit epoch moved | `pdfce-render` |
//!
...
//! weakness **from architecture** and was wrong — it declared pdfce's whole-page
...
//!
//! The raster. `pdfce-render`'s cost is already measured, already traced by the
...
///
/// ★★ The path in this project's own role documentation was `D:\Dev\temp\pdfce\`
...
fn load(path: &std::path::Path) -> Option<pdfce_core::document::Document> {
    if !path.is_file() {
        println!(
    ...
    match pdfce_core::document::Document::load(path) {
        Ok(doc) => Some(doc),
        Err(e) => {
...
fn edit_latency_the_commit_half() {
    ...
    for (label, path) in [
        ...
        let load_ms = median_ms(3, || {
            let _ = pdfce_core::document::Document::load(&path);
        });
        ...
        let session_ms = median_ms(5, || {
            let _ = pdfce_core::edit::EditSession::new(
                pdfce_core::document::Document::load(&path).expect("it loaded a moment ago"),
            );
        });
        ...
        let doc_for_view =
            pdfce_core::document::Document::load(&path).expect("it loaded a moment ago");
        let session = pdfce_core::edit::EditSession::new(doc_for_view);
        ...
        let decompose_ms = median_ms(3, || {
            ...
            let _ = pdfce_core::vector::decompose_page(
                &view,
                &pages[0],
        ...
        let move_ms = if subject.is_empty() {
            ...
            median_ms(3, || {
                ...
                let doc = pdfce_core::document::Document::load(&path).expect("it loaded");
...
fn doc_page_count(doc: &pdfce_core::document::Document) -> usize {
    pdfce_core::page_tree::pages(doc).map_or(0, |p| p.len())
}
