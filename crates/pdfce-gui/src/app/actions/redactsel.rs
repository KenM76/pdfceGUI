...
//! |---|---|
//! | the search box | **text pdfce can read as text** |
...
//! logo, a signature image and a run in a font whose encoding pdfce cannot map
...
pub fn mark_selection(doc: &mut OpenDoc, appearance: &RedactAppearance) {
    ...
    let quads: Vec<Quad> = doc
        ...
        .filter_map(|(_, canvas)| {
            ...
            Some(Quad::from_rect(pdfce_core::page_tree::Rect {
