...
/// exception in this shell (`PdfceApp::central` dispatching canvas context-menu
...
pub(crate) fn dispatch(app: &mut PdfceApp, id: &str, _actions: &mut [Action]) {
    match id {
...
/// operator who has just launched pdfce in order to combine four drawings is
...
fn merge_files(app: &mut PdfceApp) {
    ...
    // the only folder pdfce has any evidence about. `Combined.pdf` names the
