...
//! `layout.ron`, with the directory resolved by asking `pdfce-core` rather
...
/// put one.
///
/// `None` is not an error: `pdfce-core` reports it for a build with no
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("pdfce-pickstore-{name}"));
