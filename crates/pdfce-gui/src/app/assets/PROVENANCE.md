...
`blank-a4.pdf` beside this note is **redistributed** by pdfceGUI: it is
compiled into `pdfce-gui.exe` by an `include_bytes!` in
`crates/pdfce-gui/src/app/blank.rs`, so every operator who is handed a binary
...
`crates/pdfce-gui/src/icons/assets/PROVENANCE.md`, which in turn follows
`D:\Dev\pdfce`'s own asset provenance records, so that a reader moving between
the three meets one convention rather than three.

- **Source:** none. The file was **authored from scratch for pdfceGUI** on
...
- **Creator:** written for Ken Mantle's pdfceGUI, in his tree, as part of
  `file.new`.
- **Licence: the project licence — MIT**, the same grant as the rest of this
  ...
  carry.
- **Changes made by pdfce:** not applicable — there is no upstream to have
  changed.
...
file, and adding a section that reproduced pdfce's own MIT grant a second
...
1. **`/MediaBox [0 0 595.276 841.89]`** is ISO 216 **A4** — 210 × 297 mm at
   ...
   `crates/pdfce-gui/src/app/blank.rs`; it is not repeated here, because a
