...
//!
//! `pdfce-core`'s `MarkupNote::modified` takes a PDF date string **from the
//! caller** and its own note says why:
//!
//! > pdfce will not supply the timestamp, and you should not expect it to.
//!
...
//!    comparing bytes, and `pdfce-core`'s whole authoring test suite is byte
...
//!    pdfce invented one, the file would assert a fact pdfce made up. Taking
...
///
/// ```
/// let stamp = pdfce_gui::app::clock::pdf_date_utc().expect("a sane clock");
...
mod tests {
    ...
    fn the_engine_accepts_what_this_module_writes() {
        ...
        let note = pdfce_core::edit::MarkupNote::new("x").at(&stamp);
        assert!(
            note.validate().is_ok(),
