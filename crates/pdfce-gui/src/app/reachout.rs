...
//!
//! Half right, and the half that is right is this one. pdfce cannot yet
...
//! the walk gave up this says *"pdfce could not finish checking"* rather than
//! implying an all-clear.
//!
//! ## ★★ Why it is a status line and not a dialog
//!
//! Because pdfce **executes none of these**. NF4 is standing: actions are
...
use pdfce_core::forms::FormJavaScript;
...
#[must_use]
pub fn scan(session: &pdfce_core::edit::EditSession) -> ReachOut {
    let view = session.view();
    let scan = pdfce_core::forms::scan_javascript(&view);
