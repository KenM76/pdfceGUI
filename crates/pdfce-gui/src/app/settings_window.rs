...
impl PdfceApp {
    ...
    /// *shipped defaults* — it cannot reach `PdfceApp` — so a document opened
    /// by an operator who has configured anything would otherwise render under
    /// pdfce's answers rather than theirs.
    ///
...
mod tests {
    ...
    use pdfce_core::settings::{CmykIntent, UnmappableCode};
    ...
    /// it cannot reach `PdfceApp` — so the snapshot is only ever correct
    /// because `adopt` calls `adopt_settings`. A fourth open path added later
    /// that forgot to would produce a document rendered under pdfce's answers
    ...
    fn opening_a_document_adopts_the_operators_settings() {
        ...
        assert_eq!(
            ...
            "the document was opened under pdfce's own colour answer rather than the operator's"
        );
