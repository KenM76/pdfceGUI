...
/// original. `pdfce-core` refuses to guess between them, and this shell offers
...
impl PasteChords {
    ...
    ///
    /// pdfce's own first, because it is the default and because a radio group
    ...
    ///
    /// `PDFCE_DIAG_PASTE_CHORDS=acrobat` (or `new_field_first`) forces the
    ...
    /// `PDFCE_DIAG_FORM_ACCEPT`: it changes no behaviour a keyless run can
    ...
    pub fn from_environment() -> Option<Self> {
        ...
        std::env::var("PDFCE_DIAG_PASTE_CHORDS")
            .ok()
    ...
    pub const fn key(self) -> &'static str {
        match self {
            ...
            Self::PdfceOrder => "new_field_first",
            // ui-text-exempt: a file token, never displayed.
...
mod tests {
    ...
    #[test]
    fn the_acrobat_order_is_exactly_the_pdfce_order_reversed() {
        assert_eq!(
            PasteChords::PdfceOrder.new_field_chord(),
            PasteChords::AcrobatOrder.duplicate_chord()
        );
        assert_eq!(
            PasteChords::PdfceOrder.duplicate_chord(),
    ...
    fn the_default_is_the_operators_ruling() {
        assert_eq!(PasteChords::default(), PasteChords::PdfceOrder);
