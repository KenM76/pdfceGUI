...
///
/// # ★★★ Why this function exists at all, when the module header says pdfce
/// # must not go looking
///
/// Because the header's argument was never against *using* system fonts. Read
/// it again -- the objection is to pdfce **deciding silently**:
///
...
pub fn write_os_flag(on: bool) -> String {
    let mut out = String::from(
        "\n\
         ...
         # licensing question -- so pdfce does not answer it for you.\n",
    );
