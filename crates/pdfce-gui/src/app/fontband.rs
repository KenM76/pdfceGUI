...
/// [`crate::app::PdfceApp::ribbon_band`]'s renderer does for one it does not
...
/// on a document heading for a printer that cares. `pdfce-core` deliberately
...
fn colour(
    ...
    if ui.color_edit_button_srgb(&mut rgb).changed() && rgb != current && live {
        ...
        if let Ok(fill) =
            pdfce_core::text_edit::NewFill::new(pdfce_core::text_edit::FillModel::Rgb, components)
        {
