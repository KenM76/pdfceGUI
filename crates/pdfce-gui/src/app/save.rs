...
//! wrong file, which `pdfce-core`'s writer module calls the worst possible
...
//! `PdfceApp::apply` call and no frame is drawn while it is part-way through,
...
//! `PdfceApp::save_pending`, which carries it beside the code.
//!
...
//! `PdfceApp::central` dispatches the canvas's context-menu tokens from inside
...
/// applied to the save side: with `PDFCE_DIAG_SAVE_PATH` unset this opens a
...
///   permitted end-of-line forms, both left at the values pdfce has always
...
#[derive(Debug)]
enum SaveError {
    /// `pdfce-core` could not build the update. A refusal by name from the
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join("pdfce-gui-save-tests");
    ...
    /// fresh `Document::load` — the same call `PdfceApp::open_path` makes — and
