...
//!
//! This module is pdfce's answer to both questions, and nothing else. It
...
//! the directory is resolved by asking `pdfce-core` rather than by
//! computing it here:
//!
//! ```text
//! pdfce_core::settings::resolve_store()      →  StoreLocation { path, kind }
...
//!    — and `pdfce-core`'s own docs end that paragraph with *"do not
...
use pdfce_core::settings::{self, StoreKind};
...
pub struct LayoutStore {
    ...
    path: Option<PathBuf>,
    /// Which of `pdfce-core`'s two homes this is, carried so a diagnostic
...
impl Default for LayoutStore {
    ...
    /// [`crate::app::PdfceApp`] derives [`Default`], and a field whose type
...
impl LayoutStore {
    /// Load the layout from the directory `pdfce-core` puts settings in.
    ///
    ...
    /// pdfce, [`crate::app::modes::layout_for_build`] of the mode the
    ...
    ///
    /// The twin of `pdfce_core::settings::store_in`, and it exists for the
    ...
    /// the same `pdfce_core::settings::resolve_store()` call that decides
    ...
    /// Which of `pdfce-core`'s homes this store is using.
    #[must_use]
    ...
    /// [`crate::app::PdfceApp::settle_and_rasterize`] takes towards a page
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("pdfce-gui-layout-{tag}-{nanos}"));
    ...
    /// `pdfce-core` chose.**
    ///
    /// The location rule, asserted against `pdfce-core`'s own resolution
    ...
    ///
    /// `StoreKind::None` is what `pdfce-core` returns when neither the
    ...
    ///
    /// `Default` exists so `PdfceApp` can keep deriving it. The hazard that
