...
const PAGE_BOX_ID: &str = "pdfce-status-page-box"; // ui-text-exempt: widget id, never displayed

/// Where the draft text and the last commit's verdict are kept.
const PAGE_STATE_ID: &str = "pdfce-status-page-state"; // ui-text-exempt: widget id, never displayed
...
///   is refused rather than guessed at; pdfce has no locale model, and a
