...
//! overlap
//!
//! `PdfceApp::dispatch_command` has several arms of the shape
...
/// * **Cloud** is blocked on `MarkupSpec::Cloud`, which pdfce accepted on
...
mod tests {
    ...
    /// `PdfceApp::dispatch_command` by guard arms tried in order. The two
    ...
    /// `PdfceApp::dispatch_command` matching on `*_for_command(id).is_some()`
    ...
    /// `PdfceApp::dispatch_command` would claim it before its own arm, and
