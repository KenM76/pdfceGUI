...
//! and eighteen key bindings — then "the shell is data" is a description
...
//! about pdfce's manifest, and it is the one that would have failed.
//!
...
//!
//! ```text
//! cargo test -p pdfce-gui rewrite_built_in_ron -- --ignored
//! ```
//!
...
//
// `PdfceApp::new` merges only the built-in layer today, so nothing calls
