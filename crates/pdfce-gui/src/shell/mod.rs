//! # shell — pdfce's ribbon, modes, QAT and keymap, as data
//!
//! This module is pdfce's half of the contract `SHELL_FRAMEWORK.md` §1
//! sets up:
//!
...
//! learn. This module owns the *content*: which tabs pdfce has, what is on
//! them, which verbs exist and what each one is called.
//!
//! | Submodule | Holds |
//! |---|---|
//! | [`manifest`] | [`manifest::built_in`] — the whole ribbon as an `egui_shell::Shell` value: eight tabs, thirty-one groups, three modes, the QAT and the keymap. Plus [`manifest::PLANNED`], the commands deliberately **absent**. |
...
//! in the GUI, **C** in `pdfce-core`/`pdfce-cli` only, **N** nowhere. P3
