...
//! `pdfce_gui::canvas::textsel::fixture`. This check asserts **exact** values,
...
//! * the fixture is missing (regenerate it: `cargo test -p pdfce-gui
    ...
    if !pdf.exists() {
        return Err(Error::new(format!(
            "the rotated-text fixture is missing at {}. It is committed; regenerate it with \
             `cargo test -p pdfce-gui regenerate_the_rotated_text_fixture -- --ignored`. This \
    ...
    if chars != WORD.len() || quads != 1 {
        return Ok(Some(format!(
            "★ THE 90° STRING DID NOT SELECT AS ONE LINE. The sweep traced `{}` — expected \
             ...
             `chars={}` with `quads={}` is the reported defect exactly: `pdfce-core` publishes a \
