...
//! (`crates/pdfce-gui/src/ocr/fixture.rs`) argues the eight.
//!
...
//!
//! ★★ **`PDFCE_VERIFY_SCAN` overrides it, and should be used.** Point it at a
...
//! page** through `pdfce-cli ocr`. See `OPERATOR_REQUESTS.md` O93.
//!
...
///
/// Generated by `crates/pdfce-gui/src/ocr/fixture.rs`:
...
/// trace, so `PDFCE_VERIFY_SCAN` can point at a document of any length.
...
const SCAN_ENV: &str = "PDFCE_VERIFY_SCAN";
...
/// The document to drive: `PDFCE_VERIFY_SCAN` if it names a real file, else the
...
fn start_a_run(
    ...
    if !doc.is_file() {
        return Err(Error::new(format!(
            "the multi-page image-only fixture is not at {}. Generate it:\n  cargo test -p \
             pdfce-gui --lib write_synthetic_image_only_multipage -- --ignored\nOr set \
    ...
    report.note(format!(
        ...
        doc.display(),
        if real {
            "REAL SCANNED MATERIAL, named by $PDFCE_VERIFY_SCAN. Recognition quality is \
            ...
            "the committed synthetic fixture. ★ It is a RENDERED page, not a scan: no scanner \
             ...
             and establishes NOTHING about recognition quality. Set $PDFCE_VERIFY_SCAN to a \
...
mod tests {
    ...
    fn the_default_document_is_the_committed_fixture() {
        assert_eq!(SCAN_ENV, "PDFCE_VERIFY_SCAN");
